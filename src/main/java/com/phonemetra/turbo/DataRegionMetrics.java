...
import com.phonemetra.turbo.mxbean.DataRegionMetricsMXBean;

/**
 * This interface provides page memory related metrics of a specific Phonemetra TurboSQL node. The overall page memory
 ...
 * Phonemetra TurboSQL node, the metrics for every region will be collected and obtained separately.
 * <p>
 * There are two ways to get the metrics of an Phonemetra TurboSQL node.
 * <ol>
 *     <li>
 *       First, a collection of the metrics can be obtained through {@link TurboSQL#dataRegionMetrics()} method. Note that
 *       the method returns data region metrics snapshots rather than just in time memory state.
 *     </li>
 *     <li>
 *       Second, all {@link DataRegionMetrics} of a local Phonemetra TurboSQL node are visible through JMX interface. Refer to
...
public interface DataRegionMetrics {
     ...
     * This metric is enabled only for TurboSQL nodes with enabled persistence.
     *
