...
package com.phonemetra.turbo.thread;
 ...
 */
public class TurboSQLThread extends Thread {
    /** Index for unassigned thread. */
    public static final int GRP_IDX_UNASSIGNED = -1;

    /** Default thread's group. */
    private static final ThreadGroup DFLT_GRP = new ThreadGroup("turboSQL");

    /** Number of all grid threads in the system. */
    private static final AtomicLong cntr = new AtomicLong();

    /** The name of the TurboSQL instance this thread belongs to. */
    protected final String turboSQLInstanceName;
     ...
     */
    public TurboSQLThread(GridWorker worker) {
        this(worker.turboSQLInstanceName(), worker.name(), worker, GRP_IDX_UNASSIGNED, -1, GridIoPolicy.UNDEFINED);
    }

    /**
     * Creates grid thread with given name for a given TurboSQL instance.
     *
     * @param turboSQLInstanceName Name of the TurboSQL instance this thread is created for.
     * @param threadName Name of thread.
     * @param r Runnable to execute.
     */
    public TurboSQLThread(String turboSQLInstanceName, String threadName, Runnable r) {
        this(turboSQLInstanceName, threadName, r, GRP_IDX_UNASSIGNED, -1, GridIoPolicy.UNDEFINED);
    }

    /**
     * Creates grid thread with given name for a given TurboSQL instance with specified
     * thread group.
     *
     * @param turboSQLInstanceName Name of the TurboSQL instance this thread is created for.
     * @param threadName Name of thread.
     ...
     */
    public TurboSQLThread(String turboSQLInstanceName, String threadName, Runnable r, int grpIdx, int stripe, byte plc) {
        super(DFLT_GRP, r, createName(cntr.incrementAndGet(), threadName, turboSQLInstanceName));
     ...
     * @param threadName Name of thread.
     */
    protected TurboSQLThread(String turboSQLInstanceName, ThreadGroup threadGrp, String threadName) {
    ...
    /**
     * @return Related {@link GridIoPolicy} for internal TurboSQL pools.
     */
    public byte policy() {
        return plc;
    }
    ...
    /**
     * Gets name of the TurboSQL instance this thread belongs to.
     *
     * @return Name of the TurboSQL instance this thread belongs to.
     */
    public String getTurboSQLInstanceName() {
    ...
    public static boolean currentThreadCanRequestBinaryMetadata() {
        TurboSQLThread curThread = current();
    ...
    public static void onEntryProcessorEntered(boolean holdsTopLock) {
        ...
        if (curThread instanceof TurboSQLThread)
            ((TurboSQLThread)curThread).holdsTopLock = holdsTopLock;
    }
    ...
    public static void onEntryProcessorLeft() {
        ...
        if (curThread instanceof TurboSQLThread)
            ((TurboSQLThread)curThread).holdsTopLock = false;
    }
    ...
    public static void onForbidBinaryMetadataRequestSectionEntered() {
        ...
        if (curThread instanceof TurboSQLThread)
            ((TurboSQLThread)curThread).forbiddenToRequestBinaryMetadata = true;
    }
    ...
    public static void onForbidBinaryMetadataRequestSectionLeft() {
        ...
        if (curThread instanceof TurboSQLThread)
            ((TurboSQLThread)curThread).forbiddenToRequestBinaryMetadata = false;
    }

    /**
     * @return TurboSQLThread or {@code null} if current thread is not an instance of TurboSQLThread.
     */
    public static TurboSQLThread current() {
        Thread thread = Thread.currentThread();

        return thread.getClass() == TurboSQLThread.class || thread instanceof TurboSQLThread ?
            ((TurboSQLThread)thread) : null;
    }
     ...
     */
    protected static String createName(long num, String threadName, String turboSQLInstanceName) {
        return threadName + "-#" + num + (turboSQLInstanceName != null ? '%' + turboSQLInstanceName + '%' : "");
    }

    /** {@inheritDoc} */
    @Override public String toString() {
        return S.toString(TurboSQLThread.class, this, "name", getName());
    }
}
