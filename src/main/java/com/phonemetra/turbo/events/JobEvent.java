 ...
 * Grid job event.
 * <p>
 * Grid events are used for notification about what happens within the grid. Note that by design TurboSQL keeps all events
 ...
 * </li>
 * <li>
 * {@link com.phonemetra.turbo.TurboSQLEvents#localQuery(com.phonemetra.turbo.lang.TurboSQLPredicate, int...)} - querying only
 * local events stored on this local node.
 * </li>
 * <li>
 * {@link com.phonemetra.turbo.TurboSQLEvents#localListen(com.phonemetra.turbo.lang.TurboSQLPredicate, int...)} - listening to
 * local grid events (events from remote nodes not included).
 * </li>
 * </ul>
 * User can also wait for events using method {@link com.phonemetra.turbo.TurboSQLEvents#waitForLocal(com.phonemetra.turbo.lang.TurboSQLPredicate,
 * int...)}.
 * <h1 class="header">Events and Performance</h1>
 * Note that by default all events in TurboSQL are enabled and therefore generated and stored by whatever event storage
 * SPI is configured. TurboSQL can and often does generate thousands events per seconds under the load and therefore it
 ...
 * com.phonemetra.turbo.configuration.TurboSQLConfiguration#getIncludeEventTypes()} method in TurboSQL configuration. Note that
 * certain events are required for TurboSQL's internal operations and such events will still be generated but not stored
 * by event storage SPI if they are disabled in TurboSQL configuration.
 *
...
public class JobEvent extends EventAdapter {
     ...
     */
    public void taskSessionId(TurboSQLUuid sesId) {
        assert sesId != null;
     ...
     */
    public void jobId(TurboSQLUuid jobId) {
        assert jobId != null;
