 ...
 * <p>
 * TurboSQL logger could be configured either from code (for example log4j logger):
 * <pre name="code" class="java">
 *      TurboSQLConfiguration cfg = new TurboSQLConfiguration();
 *      ...
 *      URL xml = U.resolveTurboSQLUrl("config/custom-log4j.xml");
 *      TurboSQLLogger log = new Log4JLogger(xml);
 *      ...
 ...
 *          &lt;bean class="com.phonemetra.turbo.logger.jcl.JclLogger"&gt;
 *              &lt;constructor-arg type="org.apache.commons.logging.Log"&gt;
 *                  &lt;bean class="org.apache.commons.logging.impl.Log4JLogger"&gt;
 *                      &lt;constructor-arg type="java.lang.String" value="config/turboSQL-log4j.xml"/&gt;
 *                  &lt;/bean&gt;
 *              &lt;/constructor-arg&gt;
 *          &lt;/bean&gt;
 *      &lt;/property&gt;
 *      ...
 * </pre>
 * It's recommended to use TurboSQL's logger injection instead of using/instantiating
 ...
 * By default TurboSQL starts in "quiet" mode suppressing {@code INFO} and {@code DEBUG}
 ...
 * <p>
 * Note that TurboSQL's standard startup scripts <tt>$TURBOSQL_HOME/bin/turboSQL.{sh|bat}</tt> start
 * by default in "quiet" mode. Both scripts accept {@code -v} arguments to turn off "quiet" mode.
 */
@GridToStringExclude
public interface TurboSQLLogger {
    /**
     ...
     */
    TurboSQLLogger getLogger(Object ctgr);
