 ...
 * {@link TurboSQL#reentrantLock(String, boolean, boolean, boolean)}.
 * <h1 class="header">Protection from failover</h1>
 * TurboSQL lock can automatically recover from node failure.
 * <ul>
 ...
 * in case a node owning the lock fails, {@code TurboSQLException} will be thrown on every other node attempting to
 ...
 * TurboSQL lock comes in two flavours: fair and non-fair. Non-fair lock assumes no ordering should be imposed
 ...
 *
 */
public interface TurboSQLLock extends Lock, Closeable {
    /**
     ...
     *
     * @throws TurboSQLException if the node is stopped or broken in non-failoverSafe mode
     */
    @Override void lock() throws TurboSQLException;
     ...
     * the lock; or
     *
     * then {@link TurboSQLInterruptedException} is thrown and the current thread's
     * interrupted status is cleared.
     *
     * </ul>
     *
     * <p>{@link TurboSQLException} is thrown in case:
     *
     * <ul>
     *
     ...
     *
     * @throws TurboSQLInterruptedException if the current thread is interrupted
     * @throws TurboSQLException if the lock is broken in non-failoverSafe mode (any node failed while owning this lock),
     *         or local node is stopped
     */
    @Override public void lockInterruptibly() throws TurboSQLInterruptedException, TurboSQLException;
     ...
     * then {@link TurboSQLException} is thrown.
     *
     * @return {@code true} if the lock was acquired and
     *         {@code false} otherwise
     *
     * @throws TurboSQLException if node is stopped, or lock is already broken in non-failover safe mode
     */
    @Override public boolean tryLock() throws TurboSQLException;
     ...
     */
    @Override public boolean tryLock(long timeout, TimeUnit unit) throws TurboSQLInterruptedException, TurboSQLException;
     ...
     * lock is created in non-failover safe mode, then {@link TurboSQLException} is thrown.
     *
     * @throws IllegalMonitorStateException if not owned by current thread
     * @throws TurboSQLException if node is stopped, or lock is already broken in non-failover safe mode
     */
    @Override void unlock() throws TurboSQLInterruptedException;

    /**
     * Returns a {@link Condition} instance for use with this
     * {@link TurboSQLLock} instance.
     *
     * <ul>
     *
     ...
     * TurboSQLInterruptedException} will be thrown, and the thread's
     ...
     * @throws TurboSQLException if the lock is not initialized or already removed
     */
    public TurboSQLCondition getOrCreateCondition(String name) throws TurboSQLException;

    /**
     * This method is not supported in TurboSQLLock,
     * Any invocation of this method will result in {@linkplain UnsupportedOperationException}.
     * Correct way to obtain Condition object is through method {@linkplain TurboSQLLock#getOrCreateCondition(String)}
     *
     */
     ...
     * @throws TurboSQLException if the lock is not initialized or already removed
     */
    public int getHoldCount() throws TurboSQLException;
     ...
     * @throws TurboSQLException if the lock is not initialized or already removed
     */
    public boolean isHeldByCurrentThread() throws TurboSQLException;
     ...
     * @throws TurboSQLException if the lock is not initialized or already removed
     */
    public boolean isLocked() throws TurboSQLException;
     ...
     * @throws TurboSQLException if the lock is not initialized or already removed
     */
    public boolean hasQueuedThreads() throws TurboSQLException;
     ...
     * @throws TurboSQLException if the lock is not initialized or already removed
     */
    public boolean hasQueuedThread(Thread thread) throws TurboSQLException;
     ...
     * @throws TurboSQLException if the lock is not initialized or already removed
     */
    public boolean hasWaiters(TurboSQLCondition condition) throws TurboSQLException;
     ...
     * @throws TurboSQLException if the lock is not initialized or already removed
     */
    public int getWaitQueueLength(TurboSQLCondition condition) throws TurboSQLException;

    /**
     * Returns {@code true} if this lock is safe to use after node failure.
     * If not, TurboSQLInterruptedException is thrown on every other node after node failure.
     *
     ...
     * @throws TurboSQLException if the lock is not initialized or already removed
     */
    public boolean isBroken() throws TurboSQLException;
