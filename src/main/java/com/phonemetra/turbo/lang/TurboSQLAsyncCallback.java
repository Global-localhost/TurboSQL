...
import com.phonemetra.turbo.cache.query.ContinuousQuery;
 ...
 * in a callbacks. This thread pool can be configured by {@link TurboSQLConfiguration#setAsyncCallbackPoolSize(int)}.
 ...
 *     // Filter increases salary for some person on 100. Without &#064;TurboSQLAsyncCallback annotation
 ...
 *             return true;
 *
 *         turboSQL.cache("Person").put(evt.getKey(), new Person(p.getName(), p.getSalary() + 100));
 *
 *         return false;
 *     }
 * }
...
public @interface TurboSQLAsyncCallback {
    // No-op.
}
