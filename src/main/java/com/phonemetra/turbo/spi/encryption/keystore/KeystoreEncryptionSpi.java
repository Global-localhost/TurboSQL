...
package com.phonemetra.turbo.spi.encryption.keystore;
 ...
 */
public class KeystoreEncryptionSpi extends TurboSQLSpiAdapter implements EncryptionSpi {
    /**
     * Default key store entry name to store Encryption master key.
     */
    public static final String DEFAULT_MASTER_KEY_NAME = "turboSQL.master.key";
    ...
    /** {@inheritDoc} */
    @Override public KeystoreEncryptionKey create() throws TurboSQLException {
        ensureStarted();
    ...
    @Override public KeystoreEncryptionKey decryptKey(byte[] data) {
        ...
        if (!Arrays.equals(key.digest, digest))
            throw new TurboSQLException("Key is broken!");
     ...
     * Ensures spi started.
     * 
     * @throws TurboSQLException If spi not started.
     */
    private void ensureStarted() throws TurboSQLException {
        if (started())
            return;

        throw new TurboSQLException("EncryptionSpi is not started!");
    }
