...
package com.phonemetra.turbo.spi.discovery.tcp.ipfinder.vm;
...
import static com.phonemetra.turbo.TurboSQLSystemProperties.IGNITE_TCP_DISCOVERY_ADDRESSES;
...
public class TcpDiscoveryVmIpFinder extends TcpDiscoveryIpFinderAdapter {
     ...
     */
    {
        String ips = TurboSQLSystemProperties.getString(IGNITE_TCP_DISCOVERY_ADDRESSES);
    ...
    public synchronized TcpDiscoveryVmIpFinder setAddresses(Collection<String> addrs) throws TurboSQLSpiException {
        if (F.isEmpty(addrs))
            return this;
     ...
     */
    private static Collection<InetSocketAddress> address(String ipStr) throws TurboSQLSpiException {
        ipStr = ipStr.trim();
        ...
        if (colonCnt > 1) {
            ...
            if (ipStr.startsWith("[")) {
                ...
                if (ipStr.contains("]:"))
                    ...
                    throw new TurboSQLSpiException(errMsg);
            }
        }
        else {
            // IPv4 address.
        ...
        if (tokens.length == 2) {
            ...
            if (portStr.contains("..")) {
                try {
                    ...
                    throw new TurboSQLSpiException(errMsg, e);
                }
            }
            else {
                try {
