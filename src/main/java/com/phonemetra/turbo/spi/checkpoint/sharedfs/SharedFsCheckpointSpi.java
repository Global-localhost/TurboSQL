...
package com.phonemetra.turbo.spi.checkpoint.sharedfs;
...
/**
 * This class defines shared file system {@link com.phonemetra.turbo.spi.checkpoint.CheckpointSpi} implementation for
 ...
 *         &lt;bean class="com.phonemetra.turbo.spi.checkpoint.sharedfs.GridSharedFsCheckpointSpi"&gt;
...
public class SharedFsCheckpointSpi extends TurboSQLSpiAdapter implements CheckpointSpi {
    /**
     * Default checkpoint directory. Note that this path is relative to {@code TURBOSQL_HOME/work} folder
     * if {@code TURBOSQL_HOME} system or environment variable specified, otherwise it is relative to
     * {@code work} folder under system {@code java.io.tmpdir} folder.
     *
     * @see com.phonemetra.turbo.configuration.TurboSQLConfiguration#getWorkDirectory()
     */
     ...
     * path can either be absolute or relative to {@code TURBOSQL_HOME} system
     * or environment variable.
     * <p>
     * If not provided, default value is {@link #DFLT_DIR_PATH}.
     *
     * @param dirPaths Absolute or TurboSQL installation home folder relative path where checkpoints
     * will be stored.
    ...
    @Override public void spiStart(String turboSQLInstanceName) throws TurboSQLSpiException {
        ...
        if (turboSQL.configuration().getMarshaller() instanceof BinaryMarshaller)
            marsh = MarshallerUtils.jdkMarshaller(turboSQL.name());
        else
            marsh = turboSQL.configuration().getMarshaller();

        folder = getNextSharedPath();

        if (folder == null)
            throw new TurboSQLSpiException("Failed to create checkpoint directory.");

        if (!folder.isDirectory())
            throw new TurboSQLSpiException("Checkpoint directory path is not a valid directory: " + curDirPath);

        registerMBean(turboSQLInstanceName, new SharedFsCheckpointSpiMBeanImpl(this), SharedFsCheckpointSpiMBean.class);
        ...
        try {
            ...
            throw new TurboSQLSpiException("Failed to get localhost address.", e);
        }
     ...
     * @throws com.phonemetra.turbo.spi.TurboSQLSpiException Throws if initializing has filed.
     */
    @Nullable private File getNextSharedPath() throws TurboSQLSpiException {
        if (folder != null) {
            folder = null;
        ...
        while (!dirPaths.isEmpty()) {
            ...
            if (new File(curDirPath).exists())
                ...
                try {
                    folder = U.resolveWorkDirectory(turboSQL.configuration().getWorkDirectory(), curDirPath, false);
                }
                    ...
                    if (!dirPaths.isEmpty())
                        ...
                        throw new TurboSQLSpiException("Failed to resolve directory: " + curDirPath + ']', e);
                }
        ...
        if (folder != null) {
            ...
            timeoutTask = new SharedFsTimeoutTask(turboSQLInstanceName, marsh, log);
    ...
    /** {@inheritDoc} */
    @Override public byte[] loadCheckpoint(String key) throws TurboSQLSpiException {
        assert key != null;
        ...
        if (file.exists())
            try {
                ...
                throw new TurboSQLSpiException("Failed to unmarshal objects in checkpoint file: " +
                    file.getAbsolutePath(), e);
            }
            catch (IOException e) {
                throw new TurboSQLSpiException("Failed to read checkpoint file: " + file.getAbsolutePath(), e);
            }
        ...
        while (!saved) {
            ...
            try {
                ...
                if (getNextSharedPath() != null)
                    ...
                    throw new TurboSQLSpiException("Failed to write checkpoint data into file: " +
                        file.getAbsolutePath(), e);
            }
            catch (TurboSQLCheckedException e) {
                throw new TurboSQLSpiException("Failed to marshal checkpoint data into file: " +
                    file.getAbsolutePath(), e);
            }
     ...
     */
    private class SharedFsCheckpointSpiMBeanImpl extends TurboSQLSpiMBeanAdapter implements SharedFsCheckpointSpiMBean {
        /** {@inheritDoc} */
        SharedFsCheckpointSpiMBeanImpl(TurboSQLSpiAdapter spiAdapter) {
            super(spiAdapter);
        }
