...
public class ExponentialBackoffTimeoutStrategy implements TimeoutStrategy {
    ...
    /** {@inheritDoc} */
    @Override public long nextTimeout(long timeout) throws TurboSQLSpiOperationTimeoutException {
        long remainingTime = remainingTime(U.currentTimeMillis());

        if (remainingTime <= 0)
            throw new TurboSQLSpiOperationTimeoutException("Operation timed out [timeoutStrategy= " +this +"]");
