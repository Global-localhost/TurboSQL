 ...
 * execution, etc. For
 * performance reasons TurboSQL is designed to store all locally produced events
 * locally. These events can be later retrieved using either distributed query:
 * <ul>
 *      <li>{@link com.phonemetra.turbo.TurboSQLEvents#remoteQuery(com.phonemetra.turbo.lang.TurboSQLPredicate, long, int...)}</li>
 * </ul>
 * or local only query:
 * <ul>
 *      <li>{@link com.phonemetra.turbo.TurboSQLEvents#localQuery(com.phonemetra.turbo.lang.TurboSQLPredicate, int...)}</li>
 * </ul>
 ...
 */
public interface EventStorageSpi extends TurboSQLSpi {
    /**
     ...
     */
    public <T extends Event> Collection<T> localEvents(TurboSQLPredicate<T> p);

    /**
     * Records single event.
     *
     * @param evt Event that should be recorded.
     * @throws com.phonemetra.turbo.spi.TurboSQLSpiException If event recording failed for any reason.
     */
    public void record(Event evt) throws TurboSQLSpiException;
}