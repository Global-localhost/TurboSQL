...
/**
 * This class is a workaround for a versatile problems with passing arguments to the TurboSQL Windows batch launcher
 ...
 * 1. User runs turboSQL.bat with some args.<br>
 * 2. turboSQL.bat calls parseargs.bat with all that args.<br>
 ...
 * 6. turboSQL.bat uses environment variables as if they were defined by a user.
 * <p>
 *
        ...
        for (String arg : args) {
            if (arg.startsWith(JVM_OPTION_PREFIX)) {
                ...
                if (!checkJVMOptionIsSupported(jvmOpt))
                    throw new RuntimeException(JVM_OPTION_PREFIX + " JVM parameters for TurboSQL batch scripts " +
