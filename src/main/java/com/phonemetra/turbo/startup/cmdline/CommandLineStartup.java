...
package com.phonemetra.turbo.startup.cmdline;
...
import com.phonemetra.turbo.internal.util.GridConfigurationFinder;
...
import static com.phonemetra.turbo.TurboSQLSystemProperties.IGNITE_PROG_NAME;
...
import static com.phonemetra.turbo.internal.TurboSQLVersionUtils.RELEASE_DATE_STR;
import static com.phonemetra.turbo.internal.TurboSQLVersionUtils.VER_STR;

/**
 * This class defines command-line TurboSQL startup. This startup can be used to start TurboSQL
 ...
 * <p>
 * Note that scripts {@code ${TURBOSQL_HOME}/bin/turboSQL.{sh|bat}} shipped with TurboSQL use
...
public final class CommandLineStartup {
     ...
     */
    static {
        String quiteStr = System.getProperty(TurboSQLSystemProperties.IGNITE_QUIET);
        ...
        if (quiteStr != null) {
            ...
            if (!quiteStr.isEmpty()) {
                if ("false".equalsIgnoreCase(quiteStr))
                    ...
                    System.err.println("Invalid value for '" + TurboSQLSystemProperties.IGNITE_QUIET +
        ...
        try {
            ...
            String icoPath = "logo_turboSQL_128x128.png";
            ...
            final URL bannerUrl = CommandLineStartup.class.getResource("logo_turboSQL_48x48.png");
            ...
            Object aboutHndProxy = Proxy.newProxyInstance(
                ...
                new InvocationHandler() {
                    @Override public Object invoke(Object proxy, Method mtd, Object[] args) throws Throwable {
                        AboutDialog.centerShow("TurboSQL Node", bannerUrl.toExternalForm(), VER_STR,
    ...
    private static void exit(@Nullable String errMsg, boolean showUsage, int exitCode) {
        ...
        String runner = System.getProperty(IGNITE_PROG_NAME, "turboSQL.{sh|bat}");
        ...
        if (showUsage) {
            boolean turboSQL = runner.contains("turboSQL.");

            X.error(
                "Usage:",
                "    " + runner + (turboSQL ? " [?]|[path {-v}{-np}]|[-i]" : " [?]|[-v]"),
                "    Where:",
            ...
            if (turboSQL) {
                X.error(
                    ...
                    "                      Path can be absolute or relative to TURBOSQL_HOME.",
                    " ",
                    "Spring file should contain one bean definition of Java type",
                    "'com.phonemetra.turbo.configuration.TurboSQLConfiguration'. Note that bean will be",
    ...
    public static void main(String[] args) {
        if (!QUITE) {
            X.println("TurboSQL Command Line Startup, ver. " + ACK_VER_STR);
            X.println(COPYRIGHT);
            X.println();
        }
        ...
        final String turboSQLInstanceName;

        try {
            turboSQLInstanceName = G.start(cfg).name();
        }
        catch (Throwable e) {
            e.printStackTrace();
        ...
        G.addListener(new IgnitionListener() {
            @Override public void onStateChange(String name, TurboSQLState state) {
                // Skip all grids except loaded from the command line.
                if (!F.eq(turboSQLInstanceName, name))
                    return;
