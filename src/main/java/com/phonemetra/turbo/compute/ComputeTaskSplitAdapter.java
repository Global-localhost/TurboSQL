...
public abstract class ComputeTaskSplitAdapter<T, R> extends ComputeTaskAdapter<T, R> {
     ...
     * @throws TurboSQLException Thrown in case of any errors.
     *
     * @see ComputeTask#map(List, Object)
     */
    protected abstract Collection<? extends ComputeJob> split(int gridSize, T arg) throws TurboSQLException;
    ...
    @Override public final Map<? extends ComputeJob, ClusterNode> map(List<ClusterNode> subgrid, T arg) {
        ...
        if (F.isEmpty(jobs))
            throw new TurboSQLException("Split returned no jobs.");
        ...
        for (ComputeJob job : jobs) {
            ...
            if (old != null)
                throw new TurboSQLException("Failed to map task (same job instance is being mapped more than once) " +
                    "[job=" + job + ", task=" + this + ']');
        }
