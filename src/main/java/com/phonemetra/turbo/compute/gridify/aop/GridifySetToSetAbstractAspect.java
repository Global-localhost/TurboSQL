...
import com.phonemetra.turbo.compute.gridify.GridifyInput;
...
import com.phonemetra.turbo.internal.util.gridify.GridifyUtils;
...
public class GridifySetToSetAbstractAspect {
     ...
     * @throws TurboSQLCheckedException If method signature invalid..
     */
    protected void checkMethodSignature(Method mtd) throws TurboSQLCheckedException {
        ...
        if (allowedParamIdxs.isEmpty()) {
            throw new TurboSQLCheckedException("Invalid method signature. Failed to get valid method parameter types " +
        ...
        if (annParamIdxs.size() > 1) {
            throw new TurboSQLCheckedException("Invalid method signature. Only one method parameter can may annotated with @" +
        ...
        if (allowedParamIdxs.size() > 1 && annParamIdxs.isEmpty()) {
            throw new TurboSQLCheckedException("Invalid method signature. Method parameter must be annotated with @" +
                GridifyInput.class.getSimpleName() +
                "[mtdName=" + mtd.getName() + ", allowedTypes=" + GridifyUtils.getAllowedMethodParameterTypes() +
        ...
        if (!annParamIdxs.isEmpty() && !allowedParamIdxs.contains(annParamIdxs.get(0))) {
            throw new TurboSQLCheckedException("Invalid method signature. Invalid annotated parameter " +
                "[mtdName=" + mtd.getName() + ", allowedTypes=" + GridifyUtils.getAllowedMethodParameterTypes() +
                ", allowedParamIdxs=" + allowedParamIdxs + ", annParamIdxs=" + annParamIdxs + ']');
        }

        if (!GridifyUtils.isMethodReturnTypeValid(mtd.getReturnType())) {
            throw new TurboSQLCheckedException("Invalid method signature. Invalid method return type " +
     ...
     * @throws TurboSQLCheckedException If split is not allowed with current parameters.
     */
    protected void checkIsSplitToJobsAllowed(GridifyRangeArgument arg, GridifySetToSet ann) throws TurboSQLCheckedException {
        if (arg.getInputSize() == UNKNOWN_SIZE && ann.threshold() <= 0 && ann.splitSize() <= 0) {
            throw new TurboSQLCheckedException("Failed to split input method argument to jobs with unknown input size and " +
     ...
     *
     * @param compute {@link com.phonemetra.turbo.TurboSQLCompute} instance.
     ...
     * @return Result.
     * @throws TurboSQLCheckedException If execution failed.
     */
    protected Object execute(TurboSQLCompute compute, Class<?> cls, GridifyRangeArgument arg,
        GridifyNodeFilter nodeFilter, int threshold, int splitSize, long timeout) throws TurboSQLCheckedException {
