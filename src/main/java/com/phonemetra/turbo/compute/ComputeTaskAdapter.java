...
public abstract class ComputeTaskAdapter<T, R> implements ComputeTask<T, R> {
     ...
     * of {@link com.phonemetra.turbo.cluster.ClusterTopologyException} or {@link ComputeExecutionRejectedException}, which means that
     ...
     *       job results.
     * @throws TurboSQLException If handling a job result caused an error effectively rejecting
     *      a failover. This exception will be thrown out of {@link ComputeTaskFuture#get()} method.
     */
    @Override public ComputeJobResultPolicy result(ComputeJobResult res, List<ComputeJobResult> rcvd) throws TurboSQLException {
        TurboSQLException e = res.getException();
        ...
        if (e != null) {
            ...
            throw new TurboSQLException("Remote job threw user exception (override or implement ComputeTask.result(..) " +
