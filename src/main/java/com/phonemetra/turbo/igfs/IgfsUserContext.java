...
public abstract class IgfsUserContext {
     ...
     */
    public static <T> T doAs(String user, final TurboSQLOutClosure<T> c) {
        if (F.isEmpty(user))
    ...
    /**
     * Same contract that {@link #doAs(String, TurboSQLOutClosure)} has, but accepts
     ...
     * If this method is invoked outside of any {@link #doAs(String, TurboSQLOutClosure)} on the call stack, it will
     * return null. Otherwise it will return the user name set in the most lower
     * {@link #doAs(String, TurboSQLOutClosure)} call on the call stack.
