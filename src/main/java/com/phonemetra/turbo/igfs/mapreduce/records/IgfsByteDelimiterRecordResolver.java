...
public class IgfsByteDelimiterRecordResolver implements IgfsRecordResolver, Externalizable {
    ...
    @Override public IgfsFileRange resolveRecords(TurboSQLFileSystem fs, IgfsInputStream stream,
        ...
        TurboSQLBiTuple<State, Delimiter> firstDelim = findFirstDelimiter(stream, suggestedStart);
        ...
        if (curDelim != null && (curDelim.end >= suggestedStart && curDelim.end < suggestedEnd) ||
            ...
            if (curDelim == null || curDelim.end < suggestedEnd) {
                TurboSQLBiTuple<State, Delimiter> lastDelim = findFirstDelimiter(stream, suggestedEnd);
     ...
     */
    @Nullable private TurboSQLBiTuple<State, Delimiter> findFirstDelimiter(IgfsInputStream stream, long startPos)
        throws IOException {
        State state;
        Delimiter delim;
