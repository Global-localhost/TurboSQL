...
/**
 * Mode indicating how TurboSQL should wait for write replies from other nodes. Default
 * value is {@link #PRIMARY_SYNC}}, which means that TurboSQL will wait for write or commit to complete on
 ...
 * <p>
 * Write synchronization mode may be configured via {@link com.phonemetra.turbo.configuration.CacheConfiguration#getWriteSynchronizationMode()}
 * configuration property.
 */
public enum CacheWriteSynchronizationMode {
    /**
     * Flag indicating that TurboSQL should wait for write or commit replies from all nodes.
    ...
    /**
     * Flag indicating that TurboSQL will not wait for write or commit responses from participating nodes,
     ...
     * When enabled, TurboSQL will wait for write or commit to complete on {@code primary} node, but will not wait for
