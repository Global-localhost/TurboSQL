...
package com.phonemetra.turbo.cache;
...
import com.phonemetra.turbo.internal.GridKernalGateway;
import com.phonemetra.turbo.internal.GridKernalState;
...
import com.phonemetra.turbo.internal.processors.cache.GatewayProtectedCacheProxy;
...
import static com.phonemetra.turbo.TurboSQLSystemProperties.IGNITE_JCACHE_DEFAULT_ISOLATED;
...
public class CacheManager implements javax.cache.CacheManager {
    ...
    /** */
    private static final AtomicInteger turboSQLCnt = new AtomicInteger();
    ...
    /** */
    private final TurboSQLKernal turboSQL;
    ...
    public CacheManager(URI uri, CachingProvider cachingProvider, ClassLoader clsLdr, Properties props) {
        ...
        try {
            if (uri.equals(cachingProvider.getDefaultURI())) {
                ...
                cfg.setTurboSQLInstanceName("CacheManager_" + turboSQLCnt.getAndIncrement());

                cfg.setClassLoader(clsLdr);

                turboSQL = (TurboSQLKernal)IgnitionEx.start(cfg);
            }
            else
                turboSQL = (TurboSQLKernal)IgnitionEx.start(uri.toURL(), clsLdr);

            kernalGateway = turboSQL.context().gateway();
        }
    ...
    @Override public <K, V, C extends Configuration<K, V>> Cache<K, V> createCache(String cacheName, C cacheCfg)
        ...
        try {
            ...
            CacheConfiguration<K, V> turboSQLCacheCfg;

            if (cacheCfg instanceof CompleteConfiguration)
                turboSQLCacheCfg = new CacheConfiguration<>((CompleteConfiguration<K, V>)cacheCfg);
            else {
                turboSQLCacheCfg = new CacheConfiguration<>();

                turboSQLCacheCfg.setTypes(cacheCfg.getKeyType(), cacheCfg.getValueType());
            }

            turboSQLCacheCfg.setName(cacheName);

            TurboSQLCache<K, V> res = turboSQL.createCache(turboSQLCacheCfg);
            ...
            if (turboSQLCacheCfg.isManagementEnabled())
                enableManagement(cacheName, true);

            if (turboSQLCacheCfg.isStatisticsEnabled())
     ...
     * @return Cache.
     */
    @Nullable private <K, V> TurboSQLCache<K, V> getCache0(String cacheName) {
        if (cacheName == null)
            throw new NullPointerException();

        try {
            return turboSQL.cache(cacheName);
        }
    ...
    @Override public Iterable<String> getCacheNames() {
        ...
        try {
            ...
            for (TurboSQLCache<?, ?> cache : turboSQL.context().cache().publicCaches())
    ...
    @Override public void enableManagement(String cacheName, boolean enabled) {
        if(TurboSQLUtils.IGNITE_MBEANS_DISABLED)
            return;

        kernalGateway.readLock();

        try {
            TurboSQLCache<?, ?> cache = getCache0(cacheName);
    ...
    @Override public void enableStatistics(String cacheName, boolean enabled) {
        ...
        try {
            ...
            turboSQL.context().cache().cache(cacheName).context().statisticsEnabled(enabled);
        }
        finally {
            kernalGateway.readUnlock();
        }
    ...
    private void registerCacheObject(Object mxbean, String name, String beanType) {
        MBeanServer mBeanSrv = turboSQL.configuration().getMBeanServer();

        ObjectName registeredObjName = getObjectName(name, beanType);

        try {
            if (mBeanSrv.queryNames(registeredObjName, null).isEmpty()) {
                TurboSQLStandardMXBean bean = beanType.equals(CACHE_CONFIGURATION)
                    ? new TurboSQLStandardMXBean((CacheMXBean)mxbean, CacheMXBean.class)
                    : new TurboSQLStandardMXBean((CacheStatisticsMXBean)mxbean, CacheStatisticsMXBean.class);
    ...
    @Override public <T> T unwrap(Class<T> clazz) {
        ...
        if(clazz.isAssignableFrom(turboSQL.getClass()))
            return clazz.cast(turboSQL);
