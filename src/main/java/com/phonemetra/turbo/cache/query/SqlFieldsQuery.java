 ...
 * Collocation flag is used for optimization purposes. Whenever TurboSQL executes
 ...
 * as affinity key. This allows TurboSQL to execute the {@code 'limit'} clause on
...
public class SqlFieldsQuery extends Query<List<?>> {
    ...
    /** Lazy mode is default since TurboSQL v.2.8. */
     ...
     * Whenever TurboSQL executes a distributed query, it sends sub-queries to individual cluster members.
     * If you know in advance that the elements of your query selection are collocated together on the same node and
     * you group by collocated key (primary or affinity key), then TurboSQL can make significant performance and network
     ...
     * @return {@code this} For chaining.
     * @deprecated No longer used as of Phonemetra TurboSQL 2.8.
     */
    @Deprecated
     ...
     * <p>
     * By default TurboSQL attempts to fetch the whole query result set to memory and send it to the client. For small
