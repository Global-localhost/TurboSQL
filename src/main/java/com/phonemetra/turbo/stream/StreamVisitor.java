 ...
 */
public abstract class  StreamVisitor<K, V> implements StreamReceiver<K, V>, TurboSQLBiInClosure<TurboSQLCache<K, V>, Map.Entry<K, V>> {
    /** */
    ...
    /**
     * Creates a new visitor based on instance of {@link TurboSQLBiInClosure}.
     *
     * @param c Closure.
     * @return Stream visitor.
     */
    public static <K, V> StreamVisitor<K, V> from(final TurboSQLBiInClosure<TurboSQLCache<K, V>, Map.Entry<K, V>> c) {
        return new StreamVisitor<K, V>() {
            @Override public void apply(TurboSQLCache<K, V> cache, Map.Entry<K, V> entry) {
                c.apply(cache, entry);
            }
        };
    }
