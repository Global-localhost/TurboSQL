...
package com.phonemetra.turbo.internal.mem.file;
...
public class MappedFileMemoryProvider implements DirectMemoryProvider {
     ...
     */
    public MappedFileMemoryProvider(TurboSQLLogger log, File allocationPath) {
        this.log = log;
    ...
    @Override public void initialize(long[] sizes) {
        if (isInit)
            throw new TurboSQLException("Second initialization does not allowed for current provider");
        ...
        if (!allocationPath.exists()) {
            if (!allocationPath.mkdirs())
                throw new TurboSQLException("Failed to initialize allocation path (make sure directory is " +
                    "writable for the current user): " + allocationPath);
        }

        if (!allocationPath.isDirectory())
            throw new TurboSQLException("Failed to initialize allocation path (path is a file): " + allocationPath);
        ...
        if (files.length != 0) {
            ...
            for (File file : files) {
                if (!file.delete())
                    throw new TurboSQLException("Failed to delete allocated file on start (make sure file is not " +
