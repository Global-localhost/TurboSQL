...
import com.phonemetra.turbo.internal.managers.deployment.protocol.gg.GridProtocolHandler;
...
import com.phonemetra.turbo.spi.deployment.IgnoreIfPeerClassLoadingDisabled;
import org.jetbrains.annotations.Nullable;

import static com.phonemetra.turbo.configuration.DeploymentMode.CONTINUOUS;
import static com.phonemetra.turbo.configuration.DeploymentMode.ISOLATED;
import static com.phonemetra.turbo.configuration.DeploymentMode.PRIVATE;
...
public class GridDeploymentManager extends GridManagerAdapter<DeploymentSpi> {
    ...
    /** {@inheritDoc} */
    @Override public void onKernalStart0() throws TurboSQLCheckedException {
        storesOnKernalStart();
    }
    ...
    public Map<String, Class<? extends ComputeTask<?, ?>>> findAllTasks(
        @Nullable TurboSQLPredicate<? super Class<? extends ComputeTask<?, ?>>>... p) {
    ...
    private void tasks(Map<String, Class<? extends ComputeTask<?, ?>>> map, GridDeployment dep,
        TurboSQLPredicate<? super Class<? extends ComputeTask<?, ?>>>[] p) {
        assert map != null;
        assert dep != null;
     ...
     * @throws TurboSQLCheckedException If deployment failed.
     * @return Grid deployment.
     */
    @Nullable public GridDeployment deploy(Class<?> cls, ClassLoader clsLdr) throws TurboSQLCheckedException {
        if (clsLdr == null)
        ...
        if (lambdaParent != null) {
            ...
            try {
                ...
                throw new TurboSQLCheckedException("Cannot deploy parent class for lambda: " + clsName, e);
            }
        }
        ...
        if (clsLdr instanceof GridDeploymentClassLoader) {
            ...
            if ((ldr.deployMode() == ISOLATED || ldr.deployMode() == PRIVATE) &&
                ...
                throw new TurboSQLCheckedException("Attempt to deploy class loaded in ISOLATED or PRIVATE mode on node with " +
     ...
     */
    @Nullable public GridDeployment getDeployment(TurboSQLUuid ldrId) {
        if (locDep != null)
    ...
    @Nullable public GridDeployment getGlobalDeployment(
        ...
        String userVer,
        UUID sndNodeId,
        TurboSQLUuid clsLdrId,
        Map<UUID, TurboSQLUuid> participants,
        @Nullable TurboSQLPredicate<ClusterNode> nodeFilter) {
        if (locDep != null)
            return locDep;
                ...
                if (locDep != null) {
                    if (!isPerVersionMode(locDep.deployMode())) {
                        U.warn(log, "Failed to deploy class in SHARED or CONTINUOUS mode (class is locally deployed " +
                            "in some other mode). Either change TurboSQLConfiguration.getDeploymentMode() property to " +
     ...
     */
    public void addCacheParticipants(Map<UUID, TurboSQLUuid> allParticipants, Map<UUID, TurboSQLUuid> addedParticipants) {
     ...
     */
    @Nullable public TurboSQLUuid getClassLoaderId(ClassLoader ldr) {
        assert ldr != null;
     ...
     */
    private void startStores() throws TurboSQLCheckedException {
     ...
     * @throws TurboSQLCheckedException If failed.
     */
    private void storesOnKernalStart() throws TurboSQLCheckedException {
    ...
    private static class LocalDeployment extends GridDeployment {
         ...
         */
        private LocalDeployment(DeploymentMode depMode, ClassLoader clsLdr, TurboSQLUuid clsLdrId, String userVer,
        ...
        /** {@inheritDoc} */
        @Nullable @Override public Map<UUID, TurboSQLUuid> participants() {
            return null;
        }
