...
import com.phonemetra.turbo.internal.util.GridBusyLock;
import com.phonemetra.turbo.internal.util.GridByteArrayList;
...
import com.phonemetra.turbo.lang.TurboSQLNotPeerDeployable;
...
import static com.phonemetra.turbo.internal.GridTopic.TOPIC_CLASSLOAD;
...
class GridDeploymentCommunication {
     ...
     * @param log Logger.
     */
    GridDeploymentCommunication(final GridKernalContext ctx, TurboSQLLogger log) {
        assert log != null;
        ...
        if (dep != null) {
            ...
            if (!(ldr instanceof GridDeploymentClassLoader)) {
                ...
                try {
                    ...
                    if (U.getAnnotation(cls, TurboSQLNotPeerDeployable.class) != null) {
     ...
     * @throws TurboSQLCheckedException If request could not be sent.
     */
    void sendUndeployRequest(String rsrcName, Collection<ClusterNode> rmtNodes) throws TurboSQLCheckedException {
     ...
     * @throws TurboSQLCheckedException Thrown if there is no connection with remote node.
     */
    GridDeploymentResponse sendResourceRequest(final String rsrcName, TurboSQLUuid clsLdrId,
        final ClusterNode dstNode, long threshold) throws TurboSQLCheckedException {
        ...
        Object resTopic = TOPIC_CLASSLOAD.topic(TurboSQLUuid.fromUuid(ctx.localNodeId()));
        ...
        try {
            ...
            synchronized (qryMux) {
                try {
                    ...
                    throw new TurboSQLCheckedException("Got interrupted while waiting for response from node: " +
                        dstNode.id(), e);
                }
            }
