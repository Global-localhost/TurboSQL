...
class GridDeploymentClassLoader extends ClassLoader implements GridDeploymentInfo {
    ...
    /** Node ID -> Loader ID. */
    @GridToStringInclude
    private Map<UUID, TurboSQLUuid> nodeLdrMap;
    ...
    GridDeploymentClassLoader(
        ...
        Map<UUID, TurboSQLUuid> map = U.newHashMap(1);
    ...
    /** {@inheritDoc} */
    @Override public Map<UUID, TurboSQLUuid> participants() {
        synchronized (mux) {
     ...
     */
    void register(UUID nodeId, TurboSQLUuid ldrId) {
        assert nodeId != null;
        assert ldrId != null;
     ...
     */
    @Nullable TurboSQLUuid unregister(UUID nodeId) {
        assert nodeId != null;
     ...
     */
    Collection<TurboSQLUuid> registeredClassLoaderIds() {
        Collection<TurboSQLUuid> ldrIds = new LinkedList<>();

        synchronized (mux) {
            for (TurboSQLUuid ldrId : nodeLdrMap.values())
                ldrIds.add(ldrId);
        }
     ...
     */
    TurboSQLUuid registeredClassLoaderId(UUID nodeId) {
        synchronized (mux) {
     ...
     */
    boolean hasRegisteredNode(UUID nodeId, TurboSQLUuid ldrId) {
        assert nodeId != null;
        assert ldrId != null;
    ...
    @Override public Class<?> loadClass(String name) throws ClassNotFoundException {
        ...
        try {
            if (!"com.phonemetra.turbo.compute.ComputeJob".equals(name)) {
    ...
    private GridByteArrayList sendClassRequest(String name, String path) throws ClassNotFoundException {
        ...
        Map<UUID, TurboSQLUuid> nodeLdrMapCp;
        ...
        for (UUID nodeId : nodeListCp) {
            ...
            TurboSQLUuid ldrId = nodeLdrMapCp.get(nodeId);
            ...
            try {
                ...
                if (res == null) {
                    ...
                    err = new TurboSQLCheckedException(msg);
