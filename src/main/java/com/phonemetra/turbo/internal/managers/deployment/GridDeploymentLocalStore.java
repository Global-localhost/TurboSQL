...
class GridDeploymentLocalStore extends GridDeploymentStoreAdapter {
    ...
    /** {@inheritDoc} */
    @Nullable @Override public GridDeployment getDeployment(TurboSQLUuid ldrId) {
        synchronized (mux) {
    ...
    @Nullable @Override public GridDeployment explicitDeploy(Class<?> cls, ClassLoader clsLdr) throws TurboSQLCheckedException {
        try {
            ...
            if (e.getCause() instanceof TurboSQLCheckedException)
                throw (TurboSQLCheckedException)e.getCause();

            throw new TurboSQLDeploymentCheckedException("Failed to deploy class: " + cls.getName(), e);
        }
    }
