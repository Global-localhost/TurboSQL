...
import static com.phonemetra.turbo.internal.managers.communication.GridIoPolicy.SYSTEM_POOL;
 ...
 *
 */
public abstract class GridManagerAdapter<T extends TurboSQLSpi> implements GridManager {
    /** Kernal context. */
    @GridToStringExclude
    ...
    /** */
    private final Map<TurboSQLSpi, Boolean> spiMap = new IdentityHashMap<>();
    ...
    protected GridManagerAdapter(GridKernalContext ctx, T... spis) {
        ...
        for (int i = 0; i < spis.length; i++) {
            if (!U.hasAnnotation(this.spis[i].getClass(), TurboSQLSpiNoop.class))
                enabled = true;
        }
    ...
    protected final T getSpi(@Nullable String name) {
        ...
        throw new TurboSQLException("Failed to find SPI for name: " + name);
    }
     ...
     * @throws TurboSQLCheckedException If injection failed.
     */
    private void inject(TurboSQLSpi spi) throws TurboSQLCheckedException {
        if (spi instanceof TurboSQLSpiAdapter) {
            Collection<Object> injectables = ((TurboSQLSpiAdapter)spi).injectables();
     ...
     * @throws TurboSQLCheckedException If injection failed.
     */
    private void cleanup(TurboSQLSpi spi) throws TurboSQLCheckedException {
        if (spi instanceof TurboSQLSpiAdapter) {
            Collection<Object> injectables = ((TurboSQLSpiAdapter)spi).injectables();
    ...
    /** {@inheritDoc} */
    @Override public void onDisconnected(TurboSQLFuture<?> reconnectFut) throws TurboSQLCheckedException {
        for (T t : spis)
            t.onClientDisconnected(reconnectFut);
    }

    /** {@inheritDoc} */
    @Override public TurboSQLInternalFuture<?> onReconnected(boolean clusterRestarted) throws TurboSQLCheckedException {
        for (T t : spis)
     ...
     */
    protected void inject() throws TurboSQLCheckedException {
        if (injected)
            return;
     ...
     * Starts wrapped SPI.
     *
     * @throws TurboSQLCheckedException If wrapped SPI could not be started.
     */
    protected final void startSpi() throws TurboSQLCheckedException {
        Collection<String> names = U.newHashSet(spis.length);

        for (T spi : spis) {
            if (spi instanceof TurboSQLSpiAdapter)
                ((TurboSQLSpiAdapter)spi).onBeforeStart();
            ...
            try {
                ...
                if (retval != null) {
                    for (Map.Entry<String, Object> e : retval.entrySet()) {
                        if (ctx.hasNodeAttribute(e.getKey()))
                            throw new TurboSQLCheckedException("SPI attribute collision for attribute [spi=" + spi +
                                ", attr=" + e.getKey() + ']' +
                                ...
                                "Please check your TurboSQL configuration and/or SPI implementation to avoid " +
                ...
                throw new TurboSQLCheckedException("Failed to get SPI attributes.", e);
            }
            ...
            if (names.contains(spi.getName()))
                throw new TurboSQLCheckedException("Duplicate SPI name (need to explicitly configure 'setName()' property): " +
                    spi.getName());
            ...
            try {
                spi.spiStart(ctx.turboSQLInstanceName());
            }
            catch (TurboSQLSpiException e) {
                throw new TurboSQLCheckedException("Failed to start SPI: " + spi, e);
            }
     ...
     * Stops wrapped SPI.
     *
     * @throws TurboSQLCheckedException If underlying SPI could not be stopped.
     */
    protected final void stopSpi() throws TurboSQLCheckedException {
        for (T spi : spis) {
            ...
            try {
                ...
                throw new TurboSQLCheckedException("Failed to stop SPI: " + spi, e);
            }
    ...
    /** {@inheritDoc} */
    @Override public final void onKernalStart(boolean active) throws TurboSQLCheckedException {
        for (final TurboSQLSpi spi : spis) {
            try {
                spi.onContextInitialized(new TurboSQLSpiContext() {
                    ...
                    @Override public Collection<ClusterNode> remoteDaemonNodes() {
                        ...
                        return !localNode().isDaemon() ?
                            all :
                            F.view(all, new TurboSQLPredicate<ClusterNode>() {
                    ...
                    @Nullable @Override public <K, V> V put(String cacheName, K key, V val, long ttl) {
                        try {
                            if (ttl > 0) {
                                ...
                                TurboSQLCache<K, V> cache = ctx.cache().<K, V>publicJCache(cacheName).withExpiryPolicy(plc);
                    ...
                    @Override public TurboSQLNodeValidationResult validateNode(ClusterNode node) {
                        for (GridComponent comp : ctx) {
                            TurboSQLNodeValidationResult err = comp.validateNode(node);
                    ...
                    @Nullable @Override public TurboSQLNodeValidationResult validateNode(ClusterNode node, DiscoveryDataBag discoData) {
                        for (GridComponent comp : ctx) {
                            ...
                            TurboSQLNodeValidationResult err = comp.validateNode(node, discoData.newJoinerDiscoveryData(comp.discoveryDataType().ordinal()));
                     ...
                     */
                    private TurboSQLSpiException unwrapException(TurboSQLCheckedException e) {
                        // Avoid double-wrapping.
                        if (e.getCause() instanceof TurboSQLSpiException)
                            return (TurboSQLSpiException)e.getCause();

                        return new TurboSQLSpiException("Failed to execute SPI context method.", e);
                    }
                });
            }
            catch (TurboSQLSpiException e) {
                throw new TurboSQLCheckedException("Failed to initialize SPI context.", e);
            }
        }
     ...
     * @throws TurboSQLCheckedException If failed.
     */
    protected void onKernalStart0() throws TurboSQLCheckedException {
        // No-op.
    }
     ...
     * @throws TurboSQLCheckedException Thrown if given condition is {@code false}
     */
    protected final void assertParameter(boolean cond, String condDesc) throws TurboSQLCheckedException {
        if (!cond)
            throw new TurboSQLCheckedException("Grid configuration parameter failed condition check: " + condDesc);
    }
    ...
    /** {@inheritDoc} */
    @Nullable @Override public TurboSQLNodeValidationResult validateNode(ClusterNode node, DiscoveryDataBag.JoiningNodeDiscoveryData discoData) {
        return null;
    }
