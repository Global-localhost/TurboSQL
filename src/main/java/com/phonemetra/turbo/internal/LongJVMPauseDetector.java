...
import static com.phonemetra.turbo.TurboSQLSystemProperties.IGNITE_JVM_PAUSE_DETECTOR_DISABLED;
import static com.phonemetra.turbo.TurboSQLSystemProperties.IGNITE_JVM_PAUSE_DETECTOR_LAST_EVENTS_COUNT;
import static com.phonemetra.turbo.TurboSQLSystemProperties.IGNITE_JVM_PAUSE_DETECTOR_PRECISION;
import static com.phonemetra.turbo.TurboSQLSystemProperties.IGNITE_JVM_PAUSE_DETECTOR_THRESHOLD;
...
public class LongJVMPauseDetector {
    /** TurboSQL JVM pause detector threshold default value. */
     ...
     * @param log Logger.
     */
    public LongJVMPauseDetector(TurboSQLLogger log) {
        this.log = log;
    }
    ...
    /**
     * @return {@code false} if {@link TurboSQLSystemProperties#IGNITE_JVM_PAUSE_DETECTOR_DISABLED} set to {@code true},
     ...
     * wasn't occurred.
     */
    public synchronized @Nullable TurboSQLBiTuple<Long, Long> getLastLongPause() {
        int lastPauseIdx = (int)((EVT_CNT + longPausesCnt - 1) % EVT_CNT);

        if (longPausesTimestamps[lastPauseIdx] == 0)
            return null;

        return new TurboSQLBiTuple<>(longPausesTimestamps[lastPauseIdx], longPausesDurations[lastPauseIdx]);
    }
