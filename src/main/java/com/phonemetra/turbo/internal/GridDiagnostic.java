...
final class GridDiagnostic {
    ...
    /**
     * @param turboSQLInstanceName Grid instance name. Can be {@code null}.
     * @param exec Executor service.
     * @param parentLog Parent logger.
     */
    static void runBackgroundCheck(String turboSQLInstanceName, Executor exec, TurboSQLLogger parentLog) {
        assert exec != null;
        assert parentLog != null;

        final TurboSQLLogger log = parentLog.getLogger(GridDiagnostic.class);

        try {
            exec.execute(new GridWorker(turboSQLInstanceName, "grid-diagnostic-1", log) {
            ...
            exec.execute(new GridWorker(turboSQLInstanceName, "grid-diagnostic-2", log) {
                @Override public void body() {
                    try {
                        InetAddress locHost = U.getLocalHost();
            ...
            exec.execute(new GridWorker(turboSQLInstanceName, "grid-diagnostic-4", log) {
                @Override public void body() {
                    ...
                    if (!U.isSufficientlyTestedOs()) {
                        U.warn(log, "This operating system has been tested less rigorously: " + U.osString() +
                            ...
                            "turboSQL in this environment.");
                    }
                }
            });

            exec.execute(new GridWorker(turboSQLInstanceName, "grid-diagnostic-5", log) {
                @Override public void body() {
                    // Fix for GG-1075.
            ...
            exec.execute(new GridWorker(turboSQLInstanceName, "grid-diagnostic-6", log) {
                @Override public void body() {
            ...
            exec.execute(new GridWorker(turboSQLInstanceName, "grid-diagnostic-7", log) {
                @Override public void body() {
            ...
            U.error(log, "Failed to start background network diagnostics check due to thread pool execution " +
                "rejection. In most cases it indicates a severe configuration problem with TurboSQL.",
