...
package com.phonemetra.turbo.internal.jdbc.thin;
...
import com.phonemetra.turbo.cache.query.QueryCancelledException;
import com.phonemetra.turbo.internal.jdbc2.JdbcUtils;
import com.phonemetra.turbo.internal.processors.cache.query.TurboSQLQueryErrorCode;
import com.phonemetra.turbo.internal.processors.odbc.ClientListenerResponse;
import com.phonemetra.turbo.internal.processors.odbc.SqlStateCode;
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcBulkLoadBatchRequest;
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcOrderedBatchExecuteRequest;
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcOrderedBatchExecuteResult;
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcQuery;
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcQueryCancelRequest;
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcQueryExecuteRequest;
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcRequest;
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcResponse;
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcResult;
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcStatementType;
 ...
 *
 * See documentation of {@link com.phonemetra.turbo.TurboSQLJdbcThinDriver} for details.
 */
public class JdbcThinConnection implements Connection {
     ...
     * @throws SQLException In case TurboSQL client failed to start.
     */
    ...
    private synchronized void ensureConnected() throws SQLException {
        try {
            ...
            throw new SQLException("Failed to connect to TurboSQL cluster [url=" + connProps.getUrl() + ']',
    ...
    void executeNative(String sql, SqlCommand cmd, JdbcThinStatement stmt) throws SQLException {
        if (cmd instanceof SqlSetStreamingCommand) {
            ...
            if (newVal) {
                if (!cmd0.isOrdered()  && !cliIo.isUnorderedStreamSupported()) {
                    throw new SQLException("Streaming without order doesn't supported by server [remoteNodeVer="
                        + cliIo.turboSQLVersion() + ']', SqlStateCode.INTERNAL_ERROR);
                }
            ...
            throw TurboSQLQueryErrorCode.createJdbcSqlException("Unsupported native statement: " + sql,
                TurboSQLQueryErrorCode.UNSUPPORTED_OPERATION);
    }
    ...
    /**
     * @return TurboSQL server version.
     */
    TurboSQLProductVersion turboSQLVersion() {
        return cliIo.turboSQLVersion();
    }
    ...
    <R extends JdbcResult> R sendRequest(JdbcRequest req, JdbcThinStatement stmt) throws SQLException {
        ...
        try {
            ...
            if (res.status() == TurboSQLQueryErrorCode.QUERY_CANCELED && stmt != null &&
                ...
                throw new SQLTimeoutException(QueryCancelledException.ERR_MSG, SqlStateCode.QUERY_CANCELLED,
                    TurboSQLQueryErrorCode.QUERY_CANCELED);
            }
            else if (res.status() != ClientListenerResponse.STATUS_SUCCESS)
                throw new SQLException(res.error(), TurboSQLQueryErrorCode.codeToSqlState(res.status()), res.status());
            ...
            if (e instanceof SocketTimeoutException)
                ...
                throw new SQLException("Failed to communicate with TurboSQL cluster.", SqlStateCode.CONNECTION_FAILURE, e);
        }
        finally {
        ...
        void checkError() throws SQLException {
            if (err != null) {
                ...
                if (err0 instanceof SQLException)
                    ...
                    throw new SQLException("Failed to communicate with TurboSQL cluster on JDBC streaming.",
                    ...
                    if (resp.response() instanceof JdbcOrderedBatchExecuteResult) {
                        ...
                        if (res.errorCode() != ClientListenerResponse.STATUS_SUCCESS) {
                            err = new BatchUpdateException(res.errorMessage(),
                                TurboSQLQueryErrorCode.codeToSqlState(res.errorCode()),
                        ...
                        err = new SQLException(resp.error(), TurboSQLQueryErrorCode.codeToSqlState(resp.status()));
                    else
