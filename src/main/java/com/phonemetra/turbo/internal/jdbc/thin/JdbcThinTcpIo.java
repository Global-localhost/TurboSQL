...
import com.phonemetra.turbo.internal.binary.streams.BinaryHeapInputStream;
import com.phonemetra.turbo.internal.binary.streams.BinaryHeapOutputStream;
import com.phonemetra.turbo.internal.processors.cache.query.TurboSQLQueryErrorCode;
import com.phonemetra.turbo.internal.processors.odbc.ClientListenerNioListener;
import com.phonemetra.turbo.internal.processors.odbc.ClientListenerProtocolVersion;
import com.phonemetra.turbo.internal.processors.odbc.ClientListenerRequest;
import com.phonemetra.turbo.internal.processors.odbc.SqlStateCode;
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcBatchExecuteRequest;
...
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcQueryCloseRequest;
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcQueryExecuteRequest;
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcQueryFetchRequest;
import com.phonemetra.turbo.internal.processors.odbc.jdbc.JdbcQueryMetadataRequest;
...
import com.phonemetra.turbo.internal.util.HostAndPortRange;
...
import static com.phonemetra.turbo.internal.jdbc.thin.JdbcThinUtils.nullableBooleanToByte;
...
public class JdbcThinTcpIo {
    ...
    /** TurboSQL server version. */
    private TurboSQLProductVersion turboSQLVer;
    ...
    /** Current protocol version used to connection to TurboSQL. */
    ...
    public void handshake(ClientListenerProtocolVersion ver) throws IOException, SQLException {
        ...
        if (accepted) {
            if (reader.available() > 0) {
                ...
                turboSQLVer = new TurboSQLProductVersion(maj, min, maintenance, stage, ts, hash);
            }
            else
                turboSQLVer = new TurboSQLProductVersion((byte)2, (byte)0, (byte)0, "Unknown", 0L, null);
    ...
    private void handshake_2_1_0() throws IOException, SQLException {
        ...
        if (accepted) {
            turboSQLVer = new TurboSQLProductVersion((byte)2, (byte)1, (byte)0, "Unknown", 0L, null);
        ...
        try {
            if (!isUnorderedStreamSupported()) {
                throw new SQLException("Streaming without response doesn't supported by server [driverProtocolVer="
                    + CURRENT_VER + ", remoteNodeVer=" + turboSQLVer + ']', SqlStateCode.INTERNAL_ERROR);
            }
            ...
            if (stmt != null) {
                synchronized (stmt.cancellationMutex()) {
                    if (stmt.isCancelled()) {
                        ...
                        return new JdbcResponse(TurboSQLQueryErrorCode.QUERY_CANCELED, QueryCancelledException.ERR_MSG);
                    }
