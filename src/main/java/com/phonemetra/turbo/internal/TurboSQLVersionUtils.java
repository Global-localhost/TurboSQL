...
/**
 * TurboSQL version utils.
 */
public class TurboSQLVersionUtils {
    /** TurboSQL version in String form. */
    public static final String VER_STR;

    /** TurboSQL version. */
    public static final TurboSQLProductVersion VER;
     ...
     */
    static {
        VER_STR = TurboSQLProperties.get("turboSQL.version")
            .replace(".a", "-a") // Backward compatibility fix.
            .replace(".b", "-b")
            .replace(".final", "-final");

        BUILD_TSTAMP_FROM_PROPERTY = TurboSQLProperties.get("turboSQL.build");

        //Development turboSQL.properties file contains turboSQL.build = 0, so we will add the check for it.
        ...
        REV_HASH_STR = TurboSQLProperties.get("turboSQL.revision");

        RELEASE_DATE_STR = TurboSQLProperties.get("turboSQL.rel.date");

        String rev = REV_HASH_STR.length() > 8 ? REV_HASH_STR.substring(0, 8) : REV_HASH_STR;

        ACK_VER_STR = VER_STR + '#' + BUILD_TSTAMP_STR + "-sha1:" + rev;

        VER = TurboSQLProductVersion.fromString(VER_STR + '-' + BUILD_TSTAMP + '-' + REV_HASH_STR);
    }

    /**
     * Private constructor.
     */
    private TurboSQLVersionUtils() {
        // No-op.
    }
}
