...
public class GridBinaryMarshaller {
    ...
    public static BinaryContext threadLocalContext() {
        ...
        if (ctx == null) {
            TurboSQLKernal turboSQL = IgnitionEx.localTurboSQL();

            TurboSQLCacheObjectProcessor proc = turboSQL.context().cacheObjects();

            if (proc instanceof CacheObjectBinaryProcessorImpl)
                return ((CacheObjectBinaryProcessorImpl)proc).binaryContext();
            else
                throw new TurboSQLIllegalStateException("TurboSQL instance must be started with " +
                    BinaryMarshaller.class.getName() + " [name=" + turboSQL.name() + ']');
        }
