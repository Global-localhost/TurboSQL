...
import com.phonemetra.turbo.internal.binary.BinaryObjectEx;
import com.phonemetra.turbo.internal.processors.cache.query.QueryCursorEx;
import com.phonemetra.turbo.internal.processors.query.GridQueryCancel;
import com.phonemetra.turbo.internal.processors.query.GridQueryFieldMetadata;
...
import static com.phonemetra.turbo.internal.managers.communication.GridIoPolicy.MANAGEMENT_POOL;
...
public class VisorQueryUtils {
    ...
    private static String typeOf(Object o) {
        if (o != null) {
            ...
            return clazz.isArray() ? TurboSQLUtils.compact(clazz.getComponentType().getName()) + "[]"
                : TurboSQLUtils.compact(o.getClass().getName());
        }
        else
            return "n/a";
    }
     ...
     *
     * @param turboSQL TurboSQLEx instance.
     * @param qryId Query ID to get holder.
     * @return Query holder for specified query ID.
     * @throws TurboSQLException When holder is not found.
     */
    public static VisorQueryHolder getQueryHolder(final TurboSQLEx turboSQL, final String qryId) throws TurboSQLException {
        ConcurrentMap<String, VisorQueryHolder> storage = turboSQL.cluster().nodeLocalMap();

        VisorQueryHolder holder = storage.get(qryId);

        if (holder == null)
            throw new TurboSQLException(VisorQueryHolder.isSqlQuery(qryId)
     ...
     * @param qryId Query ID to get holder.
     */
    public static void removeQueryHolder(final TurboSQLEx turboSQL, final String qryId) {
        ConcurrentMap<String, VisorQueryHolder> storage = turboSQL.cluster().nodeLocalMap();
    ...
    public static void scheduleQueryStart(
        final TurboSQLEx turboSQL,
        final VisorQueryHolder holder,
        final VisorQueryTaskArg arg,
        final GridQueryCancel cancel
    ) {
        turboSQL.context().closure().runLocalSafe(() -> {
            try {
                ...
                List<FieldsQueryCursor<List<?>>> qryCursors = turboSQL
                    .context()
                    .query()
                ...
                try {
                    ...
                    VisorQueryHolder actualHolder = getQueryHolder(turboSQL, holder.getQueryID());
                    ...
                    if (meta == null)
                        ...
                        scheduleQueryHolderRemoval(turboSQL, actualHolder.getQueryID());
                    }
                }
                catch (Throwable e) {
                    U.closeQuiet(cur);
    ...
    public static void scheduleScanStart(
        ...
        turboSQL.context().closure().runLocalSafe(() -> {
            try {
                TurboSQLCache<Object, Object> c = turboSQL.cache(arg.getCacheName());
                String filterText = arg.getFilter();
                TurboSQLBiPredicate<Object, Object> filter = null;
     ...
     * @param turboSQL TurboSQLEx instance.
     */
    public static void scheduleQueryHolderRemoval(final TurboSQLEx turboSQL, final String qryId) {
        turboSQL.context().timeout().addTimeoutObject(new GridTimeoutObjectAdapter(RMV_DELAY) {
            @Override public void onTimeout() {
                ...
                if (holder != null) {
                    if (holder.isAccessed()) {
                        ...
                        scheduleQueryHolderRemoval(turboSQL, qryId);
                    }
                    else {
                        // Remove stored cursor otherwise.
                        removeQueryHolder(turboSQL, qryId);
                    }
                }
            }
        });
