...
public class VisorTxInfo extends VisorDataTransferObject {
     ...
     * @param size Size.
     */
    public VisorTxInfo(TurboSQLUuid xid, long startTime, long duration, TransactionIsolation isolation,
        TransactionConcurrency concurrency, long timeout, String lb, Collection<UUID> primaryNodes,
        TransactionState state, int size, TurboSQLUuid nearXid, Collection<UUID> masterNodeIds, AffinityTopologyVersion topVer) {
        this.xid = xid;
    ...
    /** */
    @Nullable public TurboSQLUuid getNearXid() {
        return nearXid;
    }
    ...
    public String toUserString() {
        return "    Tx: [xid=" + getXid() +
            ...
            F.transform(getPrimaryNodes(), new TurboSQLClosure<UUID, String>() {
            ...
            F.transform(getMasterNodeIds(), new TurboSQLClosure<UUID, String>() {
                @Override public String apply(UUID id) {
                    return U.id8(id);
                }
            })) +
