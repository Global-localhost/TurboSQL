...
import static com.phonemetra.turbo.internal.visor.util.VisorTaskUtils.logMapped;
...
public abstract class VisorMultiNodeTask<A, R, J> implements ComputeTask<VisorTaskArgument<A>, R> {
     ...
     * @throws TurboSQLException If mapping could not complete successfully.
     */
    protected Map<? extends ComputeJob, ClusterNode> map0(List<ClusterNode> subgrid, VisorTaskArgument<A> arg) {
        ...
        try {
            ...
            if (map.isEmpty())
                turboSQL.log().warning("No mapped jobs: [task=" + getClass().getName() +
                    ", topVer=" + turboSQL.cluster().topologyVersion() +
                    ", jobNids=" + nodeIds +
            ...
            if (debug)
                logMapped(turboSQL.log(), getClass(), map.values());
        }
    }
     ...
     * @return Task result.
     * @throws TurboSQLException If reduction or results caused an error.
     */
    @Nullable protected abstract R reduce0(List<ComputeJobResult> results) throws TurboSQLException;
