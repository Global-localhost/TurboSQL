...
import com.phonemetra.turbo.internal.cluster.DistributedBaselineConfiguration;
import com.phonemetra.turbo.internal.cluster.DetachedClusterNode;
...
public class VisorBaselineTask extends VisorOneNodeTask<VisorBaselineTaskArg, VisorBaselineTaskResult> {
    ...
    private static class VisorBaselineJob extends VisorJob<VisorBaselineTaskArg, VisorBaselineTaskResult> {
        ...
        private VisorBaselineTaskResult set0(Collection<BaselineNode> baselineTop) {
            turboSQL.cluster().setBaselineTopology(baselineTop);
        ...
        private Map<String, BaselineNode> currentBaseLine() {
            ...
            Collection<BaselineNode> baseline = turboSQL.cluster().currentBaselineTopology();
        ...
        private Map<String, BaselineNode> currentServers() {
            ...
            for (ClusterNode node : turboSQL.cluster().forServers().nodes())
                nodes.put(node.consistentId().toString(), node);
        ...
        private VisorBaselineTaskResult updateAutoAdjustmentSettings(VisorBaselineAutoAdjustSettings settings) {
            if (settings.getSoftTimeout() != null)
                turboSQL.cluster().baselineAutoAdjustTimeout(settings.getSoftTimeout());

            if (settings.getEnabled() != null)
                turboSQL.cluster().baselineAutoAdjustEnabled(settings.getEnabled());

            return collect();
        }

        /** {@inheritDoc} */
        @Override protected VisorBaselineTaskResult run(@Nullable VisorBaselineTaskArg arg) throws TurboSQLException {
