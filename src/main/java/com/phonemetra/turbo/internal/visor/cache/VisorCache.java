...
import static com.phonemetra.turbo.cache.CachePeekMode.ONHEAP;
import static com.phonemetra.turbo.cache.CachePeekMode.PRIMARY;
import static com.phonemetra.turbo.cache.CachePeekMode.BACKUP;

/**
 * Data transfer object for {@link TurboSQLCache}.
 */
public class VisorCache extends VisorDataTransferObject {
     ...
     * @throws TurboSQLCheckedException If failed to create data transfer object.
     */
    public VisorCache(TurboSQLEx turboSQL, GridCacheAdapter ca, boolean collectMetrics) throws TurboSQLCheckedException {
        assert ca != null;
        ...
        if (collectMetrics)
            metrics = new VisorCacheMetrics(turboSQL, name);

        sys = turboSQL.context().cache().systemCache(name);
