...
/**
 * Task for SQL batched update statements execution through {@link TurboSQLJdbcDriver}.
 */
class JdbcBatchUpdateTask implements TurboSQLCallable<int[]> {
    /** Serial version uid. */
     ...
     */
    public JdbcBatchUpdateTask(TurboSQL turboSQL, String cacheName, String schemaName, String sql,
     ...
     */
    private Integer doSingleUpdate(TurboSQLCache<?, ?> cache, String sqlText, List<Object> args) throws SQLException {
        ...
        if (qryCursor.isQuery()) {
            throw createJdbcSqlException(getError("Query produced result set", qry),
                TurboSQLQueryErrorCode.STMT_TYPE_MISMATCH);
        }
        ...
        if (longRes > Integer.MAX_VALUE) {
            TurboSQLLogger log = turboSQL.log();
