...
package com.phonemetra.turbo.internal.util.gridify;
 ...
 * functionality provided in {@link com.phonemetra.turbo.compute.ComputeJobAdapter} adapter, this adapter
 ...
 * in {@link com.phonemetra.turbo.compute.gridify.GridifyArgument} parameter.
 * <p>
 * Note this adapter is only useful when passing {@link com.phonemetra.turbo.compute.gridify.GridifyArgument} to
 * remote jobs. In many cases, remote jobs will not require {@link com.phonemetra.turbo.compute.gridify.GridifyArgument}
 * as they will execute their code without reflection, hence the regular
 * {@link com.phonemetra.turbo.compute.ComputeJobAdapter} should be used.
 * <p>
        ...
        try {
            ...
            if (!mtd.isAccessible())
                try {
                    ...
                    throw new TurboSQLException("Got security exception when attempting to soften access control for " +
            ...
            if (e.getTargetException() instanceof TurboSQLException)
                throw (TurboSQLException)e.getTargetException();

            throw new TurboSQLException("Failed to invoke a method due to user exception.", e.getTargetException());
        }
        catch (IllegalAccessException e) {
            throw new TurboSQLException("Failed to access method for execution.", e);
        }
        catch (NoSuchMethodException e) {
            throw new TurboSQLException("Failed to find method for execution.", e);
        }
    }
}