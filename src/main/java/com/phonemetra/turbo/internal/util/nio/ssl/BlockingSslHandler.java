...
public class BlockingSslHandler {
     ...
     */
    public boolean handshake() throws TurboSQLCheckedException, SSLException {
        if (log.isDebugEnabled())
     ...
     * @throws SSLException If failed to process SSL data.
     */
    public ByteBuffer decode(ByteBuffer buf) throws TurboSQLCheckedException, SSLException {
        appBuf.clear();
     ...
     */
    private void readFromNet() throws TurboSQLCheckedException {
        try {
            int read = ch.read(inNetBuf);

            if (read == -1)
                throw new TurboSQLCheckedException("Failed to read remote node response (connection closed).");
        }
        catch (IOException e) {
            throw new TurboSQLCheckedException("Failed to write byte to socket.", e);
        }
    }
     ...
     */
    private void writeNetBuffer() throws TurboSQLCheckedException {
        try {
