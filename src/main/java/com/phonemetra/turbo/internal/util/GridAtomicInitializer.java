...
public class GridAtomicInitializer<T> {
     ...
     */
    public T init(Callable<T> c) throws TurboSQLCheckedException {
        if (!finished) {
            synchronized (mux) {
                if (!finished) {
                    try {
        ...
        if (e != null)
            throw e instanceof TurboSQLCheckedException ? (TurboSQLCheckedException)e : new TurboSQLCheckedException(e);
     ...
     * @throws TurboSQLInterruptedCheckedException If thread was interrupted.
     */
    public boolean await() throws TurboSQLInterruptedCheckedException {
        if (!finished) {
            synchronized (mux) {
                while (!finished)
                    U.wait(mux);
