...
package com.phonemetra.turbo.internal.util.offheap.unsafe;
...
import com.phonemetra.turbo.internal.util.GridCloseableIteratorAdapter;
import com.phonemetra.turbo.internal.util.lang.GridCloseableIterator;
import com.phonemetra.turbo.internal.util.offheap.GridOffHeapEventListener;
import com.phonemetra.turbo.internal.util.offheap.GridOffHeapEvictListener;
import com.phonemetra.turbo.internal.util.offheap.GridOffHeapMap;
import com.phonemetra.turbo.internal.util.offheap.GridOffHeapOutOfMemoryException;
...
import static com.phonemetra.turbo.internal.util.offheap.GridOffHeapEvent.REHASH;
...
public class GridUnsafeMap implements GridOffHeapMap {
    ...
    /** {@inheritDoc} */
    @Nullable @Override public TurboSQLBiTuple<Long, Integer> valuePointer(int hash, byte[] keyBytes) {
    ...
    /** {@inheritDoc} */
    @Override public boolean removex(int hash, byte[] keyBytes, TurboSQLBiPredicate<Long, Integer> p) {
    ...
    /** {@inheritDoc} */
    @Override public GridCloseableIterator<TurboSQLBiTuple<byte[], byte[]>> iterator() {
        return new GridCloseableIteratorAdapter<TurboSQLBiTuple<byte[], byte[]>>() {
            private GridCloseableIterator<TurboSQLBiTuple<byte[], byte[]>> curIt;
            ...
            private void advance() throws TurboSQLCheckedException {
                curIt = null;
            ...
            @Override protected TurboSQLBiTuple<byte[], byte[]> onNext() throws TurboSQLCheckedException {
                if (curIt == null)
                    throw new NoSuchElementException();

                TurboSQLBiTuple<byte[], byte[]> t = curIt.next();
    ...
    private class Segment {
         ...
         * @return Iterator.
         */
        GridCloseableIterator<TurboSQLBiTuple<byte[], byte[]>> iterator() {
            return new GridCloseableIteratorAdapter<TurboSQLBiTuple<byte[],byte[]>>() {
                private final Queue<TurboSQLBiTuple<byte[], byte[]>> bin = new LinkedList<>();
...
//                            bin.add(F.t(Entry.readKeyBytes(entryAddr, mem), Entry.readValueBytes(entryAddr, mem)));
                ...
                @Override protected TurboSQLBiTuple<byte[], byte[]> onNext() {
                    TurboSQLBiTuple<byte[], byte[]> t = bin.poll();
...
//                            bin.add(F.t(Entry.readKeyBytes(entryAddr, mem), Entry.readValueBytes(entryAddr, mem)));
         ...
         * @return {@code True} if value was removed.
         */
        boolean removex(int hash, byte[] keyBytes, TurboSQLBiPredicate<Long, Integer> p) {
         ...
         * @return Removed value bytes.
         */
        byte[] remove(int hash, byte[] keyBytes, boolean retval, @Nullable TurboSQLBiPredicate<Long, Integer> p) {
            int relSize = 0;
            long relAddr = 0;
            long qAddr = 0;
         ...
         */
        @Nullable TurboSQLBiTuple<Long, Integer> valuePointer(int hash, byte[] keyBytes) {
            long binAddr = readLock(hash);
            ...
            try {
                ...
                while (addr != 0) {
                    if (Entry.keyEquals(addr, keyBytes, mem)) {
                        ...
                        return new TurboSQLBiTuple<>(addr + HEADER_SIZE + keyLen, valLen);
                    }
