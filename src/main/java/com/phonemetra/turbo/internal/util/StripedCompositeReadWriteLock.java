...
public class StripedCompositeReadWriteLock implements ReadWriteLock {
    ...
    private int curIdx() {
        ...
        if (curThread instanceof TurboSQLThread) {
            TurboSQLThread turboSQLThread = (TurboSQLThread)curThread;

            idx = turboSQLThread.compositeRwLockIndex();

            if (idx == TurboSQLThread.GRP_IDX_UNASSIGNED) {
                idx = IDX_GEN.incrementAndGet();

                turboSQLThread.compositeRwLockIndex(idx);
            }
        }
        else
            idx = IDX.get();
