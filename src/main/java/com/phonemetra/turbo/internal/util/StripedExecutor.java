...
public class StripedExecutor implements ExecutorService {
    ...
    /**
     * @param cnt Count.
     * @param turboSQLInstanceName Node name.
    ...
    public StripedExecutor(
        int cnt,
        String turboSQLInstanceName,
        String poolName,
        final TurboSQLLogger log,
        TurboSQLInClosure<Throwable> errHnd,
        GridWorkerListener gridWorkerLsnr,
        long failureDetectionTimeout
    ) {
        this(cnt, turboSQLInstanceName, poolName, log, errHnd, false, gridWorkerLsnr, failureDetectionTimeout);
    }
        ...
        try {
            for (int i = 0; i < cnt; i++) {
                stripes[i] = stealTasks
                    ? new StripeConcurrentQueue(turboSQLInstanceName, poolName, i, log, stripes, errHnd, gridWorkerLsnr)
                    : new StripeConcurrentQueue(turboSQLInstanceName, poolName, i, log, errHnd, gridWorkerLsnr);
            }
    ...
    private abstract static class Stripe extends GridWorker {
        ...
        private TurboSQLInClosure<Throwable> errHnd;
        ...
        public Stripe(
            ...
            super(turboSQLInstanceName, poolName + "-stripe-" + idx, log, gridWorkerLsnr);
         ...
         */
        void start() {
            thread = new TurboSQLThread(turboSQLInstanceName,
                name(),
                this,
                TurboSQLThread.GRP_IDX_UNASSIGNED,
                idx,
    ...
    private static class StripeConcurrentQueue extends Stripe {
        ...
        private static final int IGNITE_TASKS_STEALING_THRESHOLD =
            TurboSQLSystemProperties.getInteger(
                TurboSQLSystemProperties.IGNITE_DATA_STREAMING_EXECUTOR_SERVICE_TASKS_STEALING_THRESHOLD, 4);
        ...
        StripeConcurrentQueue(
            ...
            this(turboSQLInstanceName, poolName, idx, log, null, errHnd, gridWorkerLsnr);
        }
