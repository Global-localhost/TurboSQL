...
public class GridNioSslFilter extends GridNioFilterAdapter {
     ...
     */
    public GridNioSslFilter(SSLContext sslCtx, boolean directBuf, ByteOrder order, TurboSQLLogger log) {
        super("SSL filter");
    ...
    @Override public void onSessionClosed(GridNioSession ses) throws TurboSQLCheckedException {
        ...
        try {
            ...
            if (fut != null)
                fut.onDone(new TurboSQLCheckedException("SSL handshake failed (connection closed)."));
    ...
    /** {@inheritDoc} */
    @Override public void onExceptionCaught(GridNioSession ses, TurboSQLCheckedException ex)
    ...
    private GridNioSslHandler sslHandler(GridNioSession ses) {
        ...
        if (hnd == null)
            throw new TurboSQLException("Failed to process incoming message (received message before SSL handler " +
                "was created): " + ses);
