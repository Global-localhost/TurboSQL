...
public class GridNioServerBuffer {
     ...
     * @throws TurboSQLCheckedException If failed to parse message.
     */
    @Nullable public byte[] read(ByteBuffer buf) throws TurboSQLCheckedException {
        if (cnt < 0) {
            ...
            if (msgSize <= 0)
                throw new TurboSQLCheckedException("Invalid message size: " + msgSize);
