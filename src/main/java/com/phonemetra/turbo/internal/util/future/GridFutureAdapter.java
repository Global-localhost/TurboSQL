 ...
 * Future adapter.
 */
public class GridFutureAdapter<R> implements TurboSQLInternalFuture<R> {
    ...
    /** {@inheritDoc} */
    @Override public R get() throws TurboSQLCheckedException {
        return get0(ignoreInterrupts);
    }

    /** {@inheritDoc} */
    @Override public R getUninterruptibly() throws TurboSQLCheckedException {
        return get0(true);
    }

    /** {@inheritDoc} */
    @Override public R get(long timeout) throws TurboSQLCheckedException {
        // Do not replace with static import, as it may not compile.
        return get(timeout, TimeUnit.MILLISECONDS);
    }

    /** {@inheritDoc} */
    @Override public R get(long timeout, TimeUnit unit) throws TurboSQLCheckedException {
     ...
     */
    private R get0(boolean ignoreInterrupts) throws TurboSQLCheckedException {
            ...
            while (true) {
                ...
                if (Thread.interrupted()) {
                    ...
                    if (!ignoreInterrupts) {
                        ...
                        throw new TurboSQLInterruptedCheckedException("Got interrupted while waiting for future to complete.");
                    }
                }
     ...
     * @return Result.
     * @throws TurboSQLFutureTimeoutCheckedException If timeout reached before computation completed.
     * @throws TurboSQLCheckedException If error occurred.
     */
    @Nullable private R get0(boolean ignoreInterrupts, long nanosTimeout) throws TurboSQLCheckedException {
        if (isDone() || !registerWaiter(Thread.currentThread()))
            return resolve();
        ...
        throw new TurboSQLFutureTimeoutCheckedException("Timeout was reached before computation completed.");
    }
     ...
     *
     * @return Result.
     * @throws TurboSQLCheckedException If resolved to exception.
     */
    private R resolve() throws TurboSQLCheckedException {
        if (state == CANCELLED)
            throw new TurboSQLFutureCancelledCheckedException("Future was cancelled: " + this);
    ...
    private void unblock(Object waiter) {
        if (waiter instanceof Thread)
            ...
            notifyListener((TurboSQLInClosure<? super TurboSQLInternalFuture<R>>)waiter);
    }

    /** {@inheritDoc} */
    @Async.Schedule
    @Override public void listen(TurboSQLInClosure<? super TurboSQLInternalFuture<R>> lsnr) {
        if (!registerWaiter(lsnr))
            notifyListener(lsnr);
    }

    /** {@inheritDoc} */
    @Override public <T> TurboSQLInternalFuture<T> chain(
        TurboSQLClosure<? super TurboSQLInternalFuture<R>, T> doneCb
    ) {
     ...
     */
    @Nullable public TurboSQLLogger logger() {
        return null;
    }
     ...
     */
    @Async.Execute
    private void notifyListener(TurboSQLInClosure<? super TurboSQLInternalFuture<R>> lsnr) {
        assert lsnr != null;
    ...
    public void reset() {
        ...
        if (!isDone(oldState))
            throw new TurboSQLException("Illegal state");
    ...
    private static class ChainFuture<R, T> extends GridFutureAdapter<T> {
        ...
        /** */
        private TurboSQLClosure<? super TurboSQLInternalFuture<R>, T> doneCb;
