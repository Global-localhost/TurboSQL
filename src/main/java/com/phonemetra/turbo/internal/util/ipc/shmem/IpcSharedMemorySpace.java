...
import static com.phonemetra.turbo.TurboSQLSystemProperties.IGNITE_IPC_SHMEM_SPACE_DEBUG;
...
public class IpcSharedMemorySpace implements Closeable {
    ...
    public IpcSharedMemorySpace(String tokFileName, int writerPid, int readerPid, int size, boolean reader,
                                TurboSQLLogger parent) throws TurboSQLCheckedException {
     ...
     */
    public IpcSharedMemorySpace(String tokFileName, int writerPid, int readerPid, int size, boolean reader,
                                int shmemId, TurboSQLLogger parent) throws TurboSQLCheckedException {
        assert size > 0 : "Size cannot be less than 1 byte";
     ...
     * @throws TurboSQLCheckedException If space has been closed.
     * @throws IpcSharedMemoryOperationTimedoutException If operation times out.
     */
    public void write(byte[] buf, int off, int len, long timeout) throws TurboSQLCheckedException,
        ...
        try {
            if (closed.get())
                throw new TurboSQLCheckedException("Shared memory segment has been closed: " + this);
     ...
     * @throws IpcSharedMemoryOperationTimedoutException If operation times out.
     */
    public void write(ByteBuffer buf, int off, int len, long timeout) throws TurboSQLCheckedException,
            IpcSharedMemoryOperationTimedoutException {
        assert buf != null;
        assert len > 0;
     ...
     * @throws IpcSharedMemoryOperationTimedoutException If operation times out.
     */
    public int read(byte[] buf, int off, int len, long timeout) throws TurboSQLCheckedException,
            IpcSharedMemoryOperationTimedoutException {
        assert buf != null;
        assert len > 0;
        assert buf.length >= off + len;
     ...
     * @throws IpcSharedMemoryOperationTimedoutException If operation times out.
     */
    public int read(ByteBuffer buf, int off, int len, long timeout) throws TurboSQLCheckedException,
            IpcSharedMemoryOperationTimedoutException {
        assert buf != null;
        assert len > 0;
     ...
     * @throws TurboSQLCheckedException If failed.
     */
    public int unreadCount() throws TurboSQLCheckedException {
        lock.readLock().lock();
