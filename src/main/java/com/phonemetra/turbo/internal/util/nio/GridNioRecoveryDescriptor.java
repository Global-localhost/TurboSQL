...
import static com.phonemetra.turbo.TurboSQLSystemProperties.IGNITE_NIO_RECOVERY_DESCRIPTOR_RESERVATION_TIMEOUT;
...
public class GridNioRecoveryDescriptor {
    ...
    private static final long DESC_RESERVATION_TIMEOUT =
        Math.max(1_000, TurboSQLSystemProperties.getLong(IGNITE_NIO_RECOVERY_DESCRIPTOR_RESERVATION_TIMEOUT, 5_000));
    ...
    private TurboSQLBiTuple<Long, TurboSQLInClosure<Boolean>> handshakeReq;
    ...
    public void onConnected() {
        synchronized (this) {
            ...
            if (handshakeReq != null) {
                TurboSQLInClosure<Boolean> c = handshakeReq.get2();
     ...
     * @return {@code True} if reserved.
     */
    public boolean tryReserve(long id, TurboSQLInClosure<Boolean> c) {
        synchronized (this) {
            if (connected) {
                c.apply(false);
            ...
            if (reserved) {
                if (handshakeReq != null) {
                    ...
                    if (id > id0) {
                        TurboSQLInClosure<Boolean> c0 = handshakeReq.get2();

                        assert c0 != null;

                        c0.apply(false);

                        handshakeReq = new TurboSQLBiTuple<>(id, c);
                    }
                    else
                        c.apply(false);
                }
    ...
    private void notifyOnNodeLeft(SessionWriteRequest[] reqs) {
        ...
        for (SessionWriteRequest req : reqs) {
            ...
            if (req.ackClosure() != null) {
                if (cloErr == null)
                    cloErr = new TurboSQLException(e);
