...
package com.phonemetra.turbo.internal.util.offheap;
...
public interface GridOffHeapPartitionedMap {
     ...
     */
    @Nullable public TurboSQLBiTuple<Long, Integer> valuePointer(int p, int hash, byte[] keyBytes);
     ...
     * @return {@code True} if value was removed.
     */
    public boolean removex(int part, int hash, byte[] keyBytes, TurboSQLBiPredicate<Long, Integer> p);
     ...
     */
    public GridCloseableIterator<TurboSQLBiTuple<byte[], byte[]>> iterator();
     ...
     */
    public GridCloseableIterator<TurboSQLBiTuple<byte[], byte[]>> iterator(int p);
