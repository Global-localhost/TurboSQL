...
public class GridCircularBuffer<T> {
     ...
     */
    public void forEach(TurboSQLInClosure<T> c) {
        for (Item<T> t : arr) {
            T item = t.item();
     ...
     * @return Evicted object or {@code null} if nothing evicted.
     * @throws InterruptedException If interrupted.
     * @throws TurboSQLCheckedException If closure throws exception.
     */
    @Nullable public T add(T t, @Nullable TurboSQLInClosureX<T> c) throws InterruptedException, TurboSQLCheckedException {
        long idx = idxGen.getAndIncrement();
    ...
    private static class Item<V> {
         ...
         * @throws TurboSQLCheckedException If closure throws exception.
         */
        @Nullable synchronized V update(long newIdx, V newItem, long maxIdxDiff, @Nullable TurboSQLInClosureX<V> c)
            throws InterruptedException, TurboSQLCheckedException {
            assert newIdx >= 0;
