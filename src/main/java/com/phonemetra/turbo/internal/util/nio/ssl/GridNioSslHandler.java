...
package com.phonemetra.turbo.internal.util.nio.ssl;
...
import com.phonemetra.turbo.internal.util.nio.GridNioEmbeddedFuture;
import com.phonemetra.turbo.internal.util.nio.GridNioException;
...
import static com.phonemetra.turbo.internal.util.nio.ssl.GridNioSslFilter.HANDSHAKE_FUT_META_KEY;
...
class GridNioSslHandler extends ReentrantLock {
     ...
     */
    void handshake() throws TurboSQLCheckedException, SSLException {
        if (log.isDebugEnabled())
     ...
     * @throws SSLException If failed to process SSL data.
     */
    void messageReceived(ByteBuffer buf) throws TurboSQLCheckedException, SSLException {
     ...
     * @return Write future.
     */
    GridNioFuture<?> deferredWrite(ByteBuffer buf, TurboSQLInClosure<TurboSQLException> ackC) {
        assert isHeldByCurrentThread();
     ...
     */
    void flushDeferredWrites() throws TurboSQLCheckedException {
        assert isHeldByCurrentThread();
        assert handshakeFinished;
     ...
     */
    GridNioFuture<?> writeNetBuffer(TurboSQLInClosure<TurboSQLException> ackC) throws TurboSQLCheckedException {
        assert isHeldByCurrentThread();
     ...
     */
    private void unwrapData() throws TurboSQLCheckedException, SSLException {
        if (log.isDebugEnabled())
     ...
     */
    private Status unwrapHandshake() throws SSLException, TurboSQLCheckedException {
     ...
     * @throws SSLException If failed to process SSL data
     */
    private void renegotiateIfNeeded(SSLEngineResult res) throws TurboSQLCheckedException, SSLException {
