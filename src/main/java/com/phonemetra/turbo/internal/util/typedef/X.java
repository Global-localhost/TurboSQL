...
public final class X {
    ...
    @Nullable public static <T> T cloneObject(@Nullable T obj, boolean deep, boolean honorCloneable) {
        ...
        try {
            ...
            throw new TurboSQLException("Unable to clone instance of class: " + obj.getClass(), e);
        }
    }
    ...
    @Nullable private static Object deepClone(Map<Integer, Integer> identityIdxs, List<Object> clones, @Nullable Object obj,
        ...
        if (clone == null)
            throw new TurboSQLException("Failed to clone object (empty constructor could not be assigned): " + obj);
     ...
     */
    public static void waitAll(@Nullable Iterable<TurboSQLFuture<?>> futs) {
        if (F.isEmpty(futs))
            return;

        for (TurboSQLFuture fut : futs)
            fut.get();
    }
    ...
    /**
     * Tries to resolve TurboSQL installation home folder.
     *
     * @return Installation home folder.
     * @throws TurboSQLCheckedException If TurboSQL home folder was not set.
     */
    public static String resolveTurboSQLHome() throws TurboSQLCheckedException {
        String var = TurboSQLSystemProperties.getString(TURBOSQL_HOME);

        if (var != null)
            return var;
        else
            throw new TurboSQLCheckedException("Failed to resolve TurboSQL home folder " +
                "(please set 'TURBOSQL_HOME' environment or system variable)");
    }
