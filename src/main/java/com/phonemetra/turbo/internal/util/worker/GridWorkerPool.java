...
import com.phonemetra.turbo.compute.ComputeExecutionRejectedException;
...
public class GridWorkerPool {
     ...
     */
    public GridWorkerPool(Executor exec, TurboSQLLogger log) {
        assert exec != null;
        assert log != null;
     ...
     * @throws TurboSQLCheckedException Thrown if any exception occurred.
     */
    @SuppressWarnings({"ProhibitedExceptionThrown"})
    public void execute(final GridWorker w) throws TurboSQLCheckedException {
        workers.add(w);
        ...
        try {
            ...
            throw new TurboSQLCheckedException("Failed to execute worker due to runtime exception.", e);
        }
        catch (Error e) {
            workers.remove(w);
