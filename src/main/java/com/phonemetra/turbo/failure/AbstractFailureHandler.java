...
import static com.phonemetra.turbo.failure.FailureType.SYSTEM_CRITICAL_OPERATION_TIMEOUT;
import static com.phonemetra.turbo.failure.FailureType.SYSTEM_WORKER_BLOCKED;
...
public abstract class AbstractFailureHandler implements FailureHandler {
    ...
    /** {@inheritDoc} */
    @Override public boolean onFailure(TurboSQL turboSQL, FailureContext failureCtx) {
        return !ignoredFailureTypes.contains(failureCtx.type()) && handle(turboSQL, failureCtx);
    }
     ...
     * @see FailureHandler#onFailure(TurboSQL, FailureContext).
     */
    protected abstract boolean handle(TurboSQL turboSQL, FailureContext failureCtx);
