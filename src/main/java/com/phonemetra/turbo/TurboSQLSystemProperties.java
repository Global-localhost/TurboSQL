...
import com.phonemetra.turbo.configuration.CheckpointWriteOrder;
import com.phonemetra.turbo.configuration.DataStorageConfiguration;
import com.phonemetra.turbo.internal.client.GridClient;
...
import com.phonemetra.turbo.internal.processors.rest.GridRestCommand;
import com.phonemetra.turbo.internal.util.GridLogThrottle;
import com.phonemetra.turbo.stream.StreamTransformer;
import org.jetbrains.annotations.Nullable;

/**
 * Contains constants for all system properties and environmental variables in TurboSQL.
 * These properties and variables can be used to affect the behavior of TurboSQL.
 */
public final class TurboSQLSystemProperties {
    /**
     * If this system property is present the TurboSQL will include grid name into verbose log.
     *
    ...
    /**
     * If this system property is present the TurboSQL will include instance name into verbose log.
     */
     ...
     * TurboSQL instance is being restarted.
     */
     ...
     * {@link com.phonemetra.turbo.configuration.TurboSQLConfiguration#isDaemon()} configuration.
     */
    public static final String IGNITE_DAEMON = "IGNITE_DAEMON";

    /** Defines TurboSQL installation folder. */
    public static final String TURBOSQL_HOME = "TURBOSQL_HOME";
     ...
     * {@link com.phonemetra.turbo.spi.discovery.DiscoverySpiOrderSupport @GridDiscoverySpiOrderSupport(true)} will
     * be allowed.
     */
     ...
     * be performed by TurboSQL. By default, TurboSQL periodically checks for the new
     * version and prints out the message into the log if new version of TurboSQL is
     * available for download.
     */
    public static final String IGNITE_UPDATE_NOTIFIER = "IGNITE_UPDATE_NOTIFIER";

    /**
     * This system property defines interval in milliseconds in which TurboSQL will check
    ...
    /**
     * This property does not allow TurboSQL to override Jetty log configuration for REST processor.
     */
     ...
     *
     * @deprecated Should be made default in Phonemetra TurboSQL 3.0.
     */
    @Deprecated
     ...
     * of TurboSQL. In quiet mode, only warning and errors are printed into the log
     * additionally to a shortened version of standard output on the start.
     * <p>
     * Note that if you use <tt>turboSQL.{sh|bat}</tt> scripts to start TurboSQL they
    ...
    /**
     * If this property is set to {@code true} (default) and TurboSQL is launched
     ...
     * is used with auto-restarting functionality when TurboSQL is started
     * by supplied <tt>turboSQL.{bat|sh}</tt> scripts.
     */
    public static final String IGNITE_SUCCESS_FILE = "IGNITE_SUCCESS_FILE";

    /**
     * The system property sets a system-wide local IP address or hostname to be used by TurboSQL networking components.
     * Once provided, the property overrides all the default local binding settings for TurboSQL nodes.
     * <p>
     * Note, that the address can also be changed via
     * {@link com.phonemetra.turbo.configuration.TurboSQLConfiguration#setLocalHost(String)} method.
     * However, this system property has bigger priority and overrides the settings set via
     * {@link com.phonemetra.turbo.configuration.TurboSQLConfiguration}.
     */
     ...
     *
     * @see com.phonemetra.turbo.configuration.DeploymentMode
     * @see com.phonemetra.turbo.configuration.TurboSQLConfiguration#getDeploymentMode()
     */
     ...
     * Affects impact of {@link TurboSQLSystemProperties#IGNITE_DISABLE_WAL_DURING_REBALANCING} property:
    ...
    /**
     * Flag that will force TurboSQL to fill memory block with some recognisable pattern right before
     ...
     * Makes sense only for {@link com.phonemetra.turbo.spi.discovery.tcp.ipfinder.vm.TcpDiscoveryVmIpFinder}.
     */
    ...
    /**
     * Environment variable to set work directory. The property {@link com.phonemetra.turbo.configuration.TurboSQLConfiguration#setWorkDirectory} has higher
     * priority.
     */
    public static final String IGNITE_WORK_DIR = "IGNITE_WORK_DIR";

    /**
     * If this property is set to {@code true} then TurboSQL will append
     * hash code of {@link TurboSQL} class as hex string and append
     * JVM name returned by {@link RuntimeMXBean#getName()}.
     * <p>
     * This may be helpful when running TurboSQL in some application server
    ...
    /**
     * If property is set to {@code true}, then TurboSQL will disable MBeans registration.
    ...
    /** If this property is set to {@code true} then TurboSQL will log thread dump in case of partition exchange timeout. */
     ...
     * <p>
     * @deprecated Should be removed in Phonemetra TurboSQL 2.0.
     */
    @Deprecated
    ...
    /**
     * System property to specify consistent id of TurboSQL node.
     * <p>
     * Value of the system property will overwrite matched property
     * {@link com.phonemetra.turbo.configuration.TurboSQLConfiguration#setConsistentId(Serializable)} in configuration.
     */
    ...
    /**
     * Whether TurboSQL can access unaligned memory addresses.
     * <p>
    ...
    /**
     * When unsafe memory copy if performed below this threshold, TurboSQL will do it on per-byte basis instead of
    ...
    /** TurboSQL page memory concurrency level. */
    public static final String IGNITE_OFFHEAP_LOCK_CONCURRENCY_LEVEL = "IGNITE_OFFHEAP_LOCK_CONCURRENCY_LEVEL";

    /**
     * When set to {@code true}, TurboSQL switches to compatibility mode with versions that don't
    ...
    /**
     * If this property is set, then TurboSQL will use Async File IO factory by default.
     */
    public static final String IGNITE_USE_ASYNC_FILE_IO_FACTORY = "IGNITE_USE_ASYNC_FILE_IO_FACTORY";

    /**
     * If the property is set {@link com.phonemetra.turbo.internal.pagemem.wal.record.TxRecord} records
    ...
    /** Property for setup TurboSQL WAL segment sync timeout. */
    public static final String IGNITE_WAL_SEGMENT_SYNC_TIMEOUT = "IGNITE_WAL_SEGMENT_SYNC_TIMEOUT";

    /**
     * If the property is set TurboSQL will use legacy node comparator (based on node order) inste
     *
     * Default value is {@code false}.
     */
     ...
     * (such as coding mistakes in code using TurboSQL) will not be logged.
     */
    ...
    /**
     * When set to {@code true}, TurboSQL will skip partitions sizes check on partition validation after rebalance has finished.
     ...
     * But it should be disabled manually when policy is used on per entry basis to hint TurboSQL to skip this check.
     *
     * Default is {@code false}.
     */
    ...
    /**
     * When set to {@code true}, TurboSQL will allow execute DML operation (MERGE|INSERT|UPDATE|DELETE)
    ...
    /**
     * Manages the type of the implementation of the service processor (implementation of the {@link TurboSQLServices}).
     ...
     * Enforces singleton.
     */
    private TurboSQLSystemProperties() {
        // No-op.
    }
