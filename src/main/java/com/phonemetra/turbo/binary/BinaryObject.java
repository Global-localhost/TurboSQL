 ...
 * TurboSQL will always store it in memory in the binary format.
 ...
 * BinaryObject key = turboSQL.binary().toBinary(new MyKey());
 *
 ...
 * TurboSQL comes with {@link BinaryObjectBuilder} which allows to build binary objects dynamically:
 * <pre name=code class=java>
 * BinaryObjectBuilder builder = Ignition.turboSQL().binary().builder("org.project.MyObject");
 *
 ...
 * can be queried ar runtime via any of the {@link com.phonemetra.turbo.TurboSQLBinary#type(Class)}
...
public interface BinaryObject extends Serializable, Cloneable {
     ...
     * is equivalent to
     * <pre name=code class=java>
     * BinaryObjectBuilder builder = turboSQL.binary().builder(binaryObject);
     * </pre>
     *
